...
const HEADERS = {
  'User-Agent': 'Mozilla/5.0 (StockZone/2.6.2.22; Taiwan market snapshot)',
...
async function ensureMarketHistorySchema() {
  ...
  `);
  await sql.query(`ALTER TABLE market_activity_daily ADD COLUMN IF NOT EXISTS return_3_pct numeric`);
  ...
  await sql.query(`
    WITH bounds AS (
      ...
      SELECT h.*,
        ...
        LAG(close_price,3) OVER wcode AS close_3_ago,
      ...
      WHERE b.max_date IS NOT NULL AND h.trade_date >= b.max_date - INTERVAL '180 days'
      WINDOW
      ...
      SELECT *,
        ...
        CASE WHEN close_3_ago>0 THEN (close_price/close_3_ago-1)*100 ELSE NULL END AS return_3_pct,
    ...
    SELECT trade_date,stock_code,stock_name,market,trade_value,change_pct,avg_value_prev20,value_ratio_20,
           recent_value_avg_5,prior_value_avg_15,value_trend_5_15,up_value_share_5,positive_days_5,
           return_3_pct,return_5_pct,return_20_pct,baseline_days_20,recent_days_5,
    ...
    ON CONFLICT (trade_date,stock_code) DO UPDATE SET
      ...
      return_3_pct=EXCLUDED.return_3_pct, return_5_pct=EXCLUDED.return_5_pct, return_20_pct=EXCLUDED.return_20_pct,
...
async function refreshPriceSnapshotFromMarketHistory() {
  await ensureMarketHistorySchema();
  const sql = getSql();
  const rows = await sql.query(`
    WITH latest_market AS (
      ...
      SELECT h.stock_code,h.stock_name,h.market,h.trade_date,h.close_price,h.previous_close,
             h.open_price,h.high_price,h.low_price,h.source
      FROM market_daily_history h
      JOIN latest_market lm ON lm.market=h.market AND lm.trade_date=h.trade_date
    ), upserted AS (
      INSERT INTO price_snapshot
        (stock_code,stock_name,market,trade_date,close_price,previous_close,open_price,high_price,low_price,quote_time,source,updated_at)
      SELECT stock_code,stock_name,market,trade_date,close_price,previous_close,open_price,high_price,low_price,
             NULL,COALESCE(source,'market_daily_history') || ' -> snapshot',NOW()
      FROM latest_rows
      ON CONFLICT (stock_code) DO UPDATE SET
        stock_name=EXCLUDED.stock_name, market=EXCLUDED.market, trade_date=EXCLUDED.trade_date,
        close_price=EXCLUDED.close_price, previous_close=EXCLUDED.previous_close,
...
async function ensureCompanyProfileSchema() {
  ...
  // v2.6.2.33-C: official MOPS product/business enrichment cache. These columns are
  // intentionally preserved by the daily company-profile upsert.
  await sql.query(`ALTER TABLE market_company_profile ADD COLUMN IF NOT EXISTS main_business text`);
  await sql.query(`ALTER TABLE market_company_profile ADD COLUMN IF NOT EXISTS internet_address text`);
  await sql.query(`ALTER TABLE market_company_profile ADD COLUMN IF NOT EXISTS auto_business_tags jsonb NOT NULL DEFAULT '[]'::jsonb`);
  await sql.query(`ALTER TABLE market_company_profile ADD COLUMN IF NOT EXISTS business_enrich_status text`);
  await sql.query(`ALTER TABLE market_company_profile ADD COLUMN IF NOT EXISTS business_enrich_source text`);
  await sql.query(`ALTER TABLE market_company_profile ADD COLUMN IF NOT EXISTS business_enrich_checked_at timestamptz`);
  // v2.6.5.0 blind coverage engine: market-topic evidence is stored separately from raw business tags.
  await sql.query(`ALTER TABLE market_company_profile ADD COLUMN IF NOT EXISTS auto_market_topics jsonb NOT NULL DEFAULT '[]'::jsonb`);
  await sql.query(`ALTER TABLE market_company_profile ADD COLUMN IF NOT EXISTS business_enrich_version text`);
  await sql.query(`ALTER TABLE market_company_profile ADD COLUMN IF NOT EXISTS business_enrich_evidence jsonb NOT NULL DEFAULT '{}'::jsonb`);
...
module.exports = {
  ...
  upsertPriceRows, upsertPriceAndHistoryRows, upsertMarketHistoryRows, refreshMarketActivityFactors, refreshPriceSnapshotFromMarketHistory, replaceDisposalMarket
};
