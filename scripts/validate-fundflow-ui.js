...
for(const token of ['data-fundflow-scope="technology-upstream"','data-fundflow-scope="technology-midstream"','data-fundflow-scope="technology-downstream"','data-fundflow-browser-scope="technology-upstream"','fundflowTechStage','fundflowScopeBucket(item)!==fundflowBrowserScope']) if(!html.includes(token)&&!app.includes(token)) errors.push(`missing v2.6.5.19 classification token ${token}`);
...
for(const token of ['new Set(["technology-upstream","technology-midstream","technology-downstream"])','fundflowScopes.size===1','fundflowScopes.delete(key)','fundflowScopes.add(key)']) if(!app.includes(token))errors.push(`multi-select classification rule missing: ${token}`);
if(errors.length){console.error('Fundflow UI validation FAILED');for(const e of errors)console.error('-',e);process.exit(1)}
console.log(`Fundflow UI/upgrade validation PASS — ${ids.length} unique DOM ids, multi-select upstream/midstream/downstream/traditional controls + fan assertions present`);
