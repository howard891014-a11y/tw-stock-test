...
const RULES = Object.freeze([
  ...
  ['motor_driver_ic', /(?:無刷(?:直流)?|bldc)?\s*馬達.{0,8}(?:驅動|控制)(?:ic|晶片|模組)?|motor\s*driver|bldc.{0,8}(?:driver|controller)/iu],
  ['optical_comm_ic', /光通訊(?:ic|晶片)|optical communication ic/iu],
  ['image_sensor_ic', /影像感測(?:ic|晶片|積體電路)|cmos image sensor|\bcis\b/iu],
  ...
  ['display_controller_ic', /時序控制(?:ic|晶片|積體電路)?|\btcon\b|display\s*controller(?:\s*ic)?/iu],
  ['bios_firmware', /\bbios\b|韌體|firmware|基本輸入輸出系統/iu],
  ['chipset', /晶片組|chipset/iu],
  ['ic_module', /(?:積體電路|\bic\b).{0,6}模組|ic\s*module/iu],
  ['bmc', /\bbmc\b|基板管理控制(?:器|系統)|baseboard\s*management\s*controller/iu],
  ...
  ['semiconductor_chemicals_materials', /半導體.*(?:材料|化學)|電子級(?:化學品|材料)|高純度(?:電子|半導體).*(?:化學品|材料)/iu],

  // semiconductor equipment / packaging / test
  ['wet_process_equipment', /(?:晶圓|半導體).*(?:清洗|濕式|濕製程).*設備|濕製程設備|wet\s*process/iu],
  ...
  ['probe_card', /探針卡|晶圓測試卡|probe card/iu],
  ...
  ['optical_lens', /光學鏡頭|光學鏡片|車用鏡頭|攝影鏡頭|相機鏡頭|optical\s*lens|camera\s*lens/iu],
  ...
  ['fiber_component', /光纖(?:.{0,8})?(?:元件|組件|連接器|被動元件)|fiber\s*optic/iu],
  ...
  ['pneumatic_component', /氣動(?:元件|零組件|控制元件|產品|系統)|pneumatic(?:s| component)?/iu],
  ...
  ['surface_treatment', /表面處理|精密電鍍|電鍍(?:加工|處理|代工|製程)?|surface\s*treatment|plating/iu],
  ['card_reader', /讀卡機|讀卡器|smart\s*card\s*reader|rfid\s*reader/iu],
  ['pos_payment_terminal', /pos(?:系統|機|終端)?|銷售點終端|支付終端|刷卡機/iu],
  ['hdd_component', /硬碟(?:機)?零組件|磁頭|硬碟夾取臂|disk\s*clamp|hdd.*(?:component|part)|硬碟機.*零件/iu],
  ...
  ['ferrite_magnetic_component', /鐵氧體|磁性元件|磁鐵芯|磁芯|ferrite|magnetic\s*component/iu],
  ['emc_protection_component', /\bpptc\b|自復式保險絲|正溫度係數熱敏電阻|高分子正溫度係數|線路保護元件|esd保護元件/iu],
  ['hinge', /\bhinge\b|轉軸|機構樞紐/iu],
  ['water_treatment_engineering', /(?:廢\s*\(?污\)?水|污水|廢水)(?:處理|工程|處理業)?|水處理(?:工程|設備|系統)?|環境保護工程|環保工程/iu],
...
function classifyBusinessText(text,{industryCode='',code='',allowBroad=false}={}){
  ...
  // Broad industry tags normally stay fallback-only, but some cross-industry businesses (notably defense)
  // must remain additive even when another fine tag already matched. Otherwise a company that says
  // "航太國防 + 工業電腦" silently loses 國防軍工.
  const additiveBroad=[];
  if(t){
    for(const [id,re] of BROAD_RULES){
      if(!re.test(t)||!validVotableTag(id))continue;
      if(allowBroad||id==='defense')additiveBroad.push(id);
    }
  }
  const merged=uniq([...unique,...additiveBroad]);
  if(merged.length)return merged.slice(0,28);
  return [];
}
...
function blindCoverageForProfile(row,text,{allowBroad=false}={}){
  ...
  // Auto tags are derived data, not permanent seeds. Recompute them from the current
  // official business text on every engine revision so stale/incorrect auto labels can self-heal.
  const discoveredRaw=classifyBusinessText(mainBusiness,{industryCode:row.industry_code,code:row.stock_code,allowBroad});
  const rawTags=uniq(discoveredRaw);
  ...
  const marketTopics=uniq(discoveredTopics);
  return {
...
async function applyCachedBlindCoverage(sql,rows){
  ...
  if(!updates.length)return {updated:0,addedRaw:0,addedTopics:0,removedRaw:0,removedTopics:0};
  await sql.query(`
    WITH incoming AS (
  ...
  let addedRaw=0,addedTopics=0,removedRaw=0,removedTopics=0;
  for(const u of updates){
    const old=rows.find(x=>String(x.stock_code)===u.stock_code);
    const oldRaw=parseAutoBusinessTags(old?.auto_business_tags),oldTopics=parseAutoMarketTopics(old?.auto_market_topics);
    addedRaw+=Math.max(0,u.auto_business_tags.length-oldRaw.length);
    addedTopics+=Math.max(0,u.auto_market_topics.length-oldTopics.length);
    removedRaw+=Math.max(0,oldRaw.length-u.auto_business_tags.length);
    removedTopics+=Math.max(0,oldTopics.length-u.auto_market_topics.length);
  }
  return {updated:updates.length,addedRaw,addedTopics,removedRaw,removedTopics};
}
...
function needsNetworkRefresh(row,{retry=false}={}){
  if(!clean(row?.main_business)){
    // A failed MOPS request must not be hammered again immediately. The catch-up runner
    // will automatically wait for this short cooldown while untouched companies continue.
    if(!retry&&row?.business_enrich_status==='error'&&staleMs(row)<ERROR_RETRY_AFTER_MS)return false;
    return true;
  }
...
async function runBusinessEnrichment({limit=60,delayMs=0,maxRunMs=46000,retry=false,concurrency=6}={}){
  ...
  const workers=Array.from({length:Math.max(1,Math.min(10,Number(concurrency)||6))},async()=>{
    while(true){
      ...
      try{
        ...
        await sql.query(`UPDATE market_company_profile SET business_enrich_status='error',business_enrich_source='MOPS t05st03 + blind coverage',business_enrich_version=$2,business_enrich_evidence=$3::jsonb,business_enrich_checked_at=NOW() WHERE stock_code=$1`,[p.stock_code,BLIND_COVERAGE_VERSION,JSON.stringify({version:BLIND_COVERAGE_VERSION,error:errorText})]).catch(()=>{});
        results.push({code:p.stock_code,name:p.stock_name,status:'error',rawTags:[],marketTopics:[],error:errorText});
      }
  ...
  const mappingChanged=cached.addedRaw>0||cached.addedTopics>0||cached.removedRaw>0||cached.removedTopics>0||results.some(x=>x.discoveredRaw?.length||x.discoveredTopics?.length);
  ...
  return {
    ...
    cachedRemovedRaw:cached.removedRaw,cachedRemovedTopics:cached.removedTopics,
...
async function readBlindCoverageExport(){
  const { ensureCompanyProfileSchema } = require('./sync-common');
  const { getSql } = require('./db');
  await ensureCompanyProfileSchema();
  const sql=getSql();
  const rows=await loadProfiles(sql);
  const items=[];
  for(const p of rows){
    const current=resolveCompanyBusinessTags(profileForResolver(p));
    const topicLinks=marketTopicLinks(current.tags,p.stock_name,p.stock_code,{
      autoMarketTopics:p.auto_market_topics,
      mainBusiness:clean(p.main_business)
    });
    items.push({
      code:String(p.stock_code||''),
      name:clean(p.stock_name),
      market:clean(p.market),
      industryCode:clean(p.industry_code),
      industry:clean(p.industry),
      mainBusiness:clean(p.main_business),
      website:clean(p.internet_address),
      enrichStatus:clean(p.business_enrich_status),
      enrichVersion:clean(p.business_enrich_version),
      checkedAt:p.business_enrich_checked_at||null,
      rawTags:current.tags.map(t=>({id:t.id,name:t.name||t.id})),
      marketTopics:topicLinks.map(t=>({id:t.id,name:t.name||t.id,origin:t.origin||'',sourceTagIds:t.sourceTagIds||[]})),
      autoBusinessTags:parseAutoBusinessTags(p.auto_business_tags),
      autoMarketTopics:parseAutoMarketTopics(p.auto_market_topics)
    });
  }
  return {
    ok:true,view:'blind-export',version:BLIND_COVERAGE_VERSION,
    generatedAt:new Date().toISOString(),total:items.length,
    withMainBusiness:items.filter(x=>x.mainBusiness).length,items
  };
}
...
async function readPendingBusinessEnrichment({limit=200}={}){
  ...
  const now=Date.now();
  const cooling=pending.filter(p=>!clean(p.main_business)&&p.business_enrich_status==='error'&&Number.isFinite(new Date(p.business_enrich_checked_at||0).getTime())&&(now-new Date(p.business_enrich_checked_at).getTime())<ERROR_RETRY_AFTER_MS);
  let retryAfterMs=0;
  for(const p of cooling){
    const age=now-new Date(p.business_enrich_checked_at).getTime();
    retryAfterMs=Math.max(retryAfterMs,Math.max(0,ERROR_RETRY_AFTER_MS-age));
  }
  const counts={};for(const p of pending){const k=String(p.industry_code||'');counts[k]=(counts[k]||0)+1;}
  return {ok:true,mode:'blind-all-company-scan',version:BLIND_COVERAGE_VERSION,pending:pending.length,withMainBusiness:rows.filter(x=>clean(x.main_business)).length,total:rows.length,
    coolingErrors:cooling.length,retryAfterMs:Math.ceil(retryAfterMs),byIndustry:counts,
    items:pending.slice(0,Math.max(1,Math.min(500,Number(limit)||200))).map(p=>({code:p.stock_code,name:p.stock_name,market:p.market,industryCode:p.industry_code,industry:p.industry,status:p.business_enrich_status||'',version:p.business_enrich_version||'',checkedAt:p.business_enrich_checked_at||null,mainBusiness:p.main_business||'',error:(p.business_enrich_evidence&&typeof p.business_enrich_evidence==='object'?p.business_enrich_evidence.error:'')||''}))};
}
...
module.exports={
  ...
  runBusinessEnrichment,readBlindCoverageAudit,readBlindCoverageExport,readPendingBusinessEnrichment,readUnclassifiedProfiles
};
