// StockZone v2.6.2.33-C4 — official MOPS business-profile enrichment
//
// Purpose:
...
function classifyBusinessText(text,{industryCode='',code=''}={}){
...
async function runBusinessEnrichment({limit=30,delayMs=120,maxRunMs=42000,retry=false}={}){
  ...
  for(let i=0;i<picked.length;i++){
    ...
    try{
      ...
      const tags=classifyBusinessText(detail.mainBusiness,{industryCode:p.industry_code,code:p.stock_code});
...
module.exports={RULES,CODE_OVERRIDES,classifyBusinessText,fetchMopsProfile,runBusinessEnrichment,readPendingBusinessEnrichment};
