// StockZone v2.6.2.31 — Business Fund-Flow XY Engine v2 + Phase Transition Engine v1
...
function catalogScope(tag){
  if(!tag)return 'other';
  if(tag.id==='other_industry')return 'other';
  if(catalogTagIsTechnology(tag.id))return tag.resolution==='fallback'?'other':'technology-fine';
...
function buildBusinessBrowserCatalog(profiles=[],snapshot={}){
  ...
  return {
    ...
    counts:{
      totalDefinitions:items.length,technologyFineDefinitions:count(x=>x.scope==='technology-fine'),technologyFallbackDefinitions:count(x=>x.resolution==='fallback'),
      traditionalDefinitions:count(x=>x.scope==='traditional-coarse'),otherDefinitions:count(x=>x.scope==='other'),representedDefinitions:count(x=>x.represented),withXY:count(x=>x.xyEligible),withoutXY:count(x=>!x.xyEligible),
