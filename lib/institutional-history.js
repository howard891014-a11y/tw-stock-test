// StockZone v2.6.5.10 — persisted official institutional-flow history layer.
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      const r = await fetch(url, {
        ...
        redirect: 'follow',
        headers: {
          'User-Agent': 'Mozilla/5.0 (StockZone/2.6.5.10; Taiwan institutional flow)',
...
async function existingInstitutionalDates(market) {
  await ensureInstitutionalHistorySchema();
  const sql = getSql();
  const rows = await sql.query(`SELECT DISTINCT trade_date::text AS d FROM institutional_trading_daily WHERE market=$1 ORDER BY trade_date DESC LIMIT 80`, [market]);
  return new Set(rows.map(x => String(x.d).slice(0,10)));
}

async function runInstitutionalBackfill({ maxNewDays = 4, scanCalendarDays = 90, maxRunMs = 42000 } = {}) {
  await ensureInstitutionalHistorySchema();
  const started = Date.now();
  const existing = { '上市': await existingInstitutionalDates('上市'), '上櫃': await existingInstitutionalDates('上櫃') };
  ...
  for (let scanned = 0; scanned < scanCalendarDays; scanned++, iso = isoAddDays(iso, -1)) {
    ...
    for (const market of ['上市','上櫃']) {
      ...
      if (Date.now() - started > maxRunMs) break;
      try {
        const rows = market === '上市' ? await fetchTwseInstitutionalDay(iso) : await fetchTpexInstitutionalDay(iso);
  ...
  return {
    ...
    source: 'institutional_trading_backfill', addedDates, errors: errors.slice(0,12), elapsedMs: Date.now() - started
  };
}

module.exports = {
  ensureInstitutionalHistorySchema, upsertInstitutionalRows, readInstitutionalForStock,
  institutionalHistoryIsFresh, runInstitutionalSync, runInstitutionalBackfill,
  _test: {
