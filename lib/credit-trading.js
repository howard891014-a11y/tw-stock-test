// StockZone v2.6.5.16 — official credit-trading history layer with 250/500D research backfill.
...
const TPEX_MARGIN_MODERN = 'https://www.tpex.org.tw/www/zh-tw/margin/balance';
const TPEX_SBL_MODERN = 'https://www.tpex.org.tw/www/zh-tw/margin/sbl';
const TPEX_MARGIN_LEGACY = 'https://www.tpex.org.tw/web/stock/margin_trading/margin_balance/margin_bal_result.php';
const TPEX_SBL_LEGACY = 'https://www.tpex.org.tw/web/stock/margin_trading/margin_sbl/margin_sbl_result.php';
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      const r = await fetch(url, {
        ...
        redirect: 'follow',
        headers: {
          'User-Agent': 'Mozilla/5.0 (StockZone/2.6.5.16; Taiwan credit trading)',
...
function tpexPayloadRows(payload, minCols = 1) {
  if (Array.isArray(payload?.aaData)) return payload.aaData;
  const tables = Array.isArray(payload?.tables) ? payload.tables : [];
  const table = tables.find(t => Array.isArray(t?.data) && t.data.some(r => Array.isArray(r) && r.length >= minCols));
  return Array.isArray(table?.data) ? table.data : [];
}

function tpexPayloadDate(payload, requestedIso = '') {
  const tables = Array.isArray(payload?.tables) ? payload.tables : [];
  const raw = payload?.reportDate || payload?.date || tables.find(t => t?.date)?.date || '';
  return rocToIso(raw) || requestedIso;
}

function parseTpexMargin(payload, requestedIso) {
  const data = tpexPayloadRows(payload, 18);
  if (!payload || !data.length) return [];
  const reportIso = tpexPayloadDate(payload, requestedIso);
...
function parseTpexSbl(payload, requestedIso) {
  const data = tpexPayloadRows(payload, 14);
  if (!payload || !data.length) return [];
  const reportIso = tpexPayloadDate(payload, requestedIso);
...
async function fetchTpexParsed(urls, parser, iso) {
  const errors = [];
  for (const url of urls) {
    try {
      const rows = parser(await fetchJson(url, { retries: 0, timeoutMs: 6500 }), iso);
      if (rows.length) return { rows, url, errors };
    } catch (e) {
...
async function fetchTpexCreditDay(iso) {
  // TPEx has both current /www routes and legacy result.php routes.  The two
  // current reports are not perfectly consistent about accepted date formats,
  // so try ROC + Gregorian forms before the legacy compatibility endpoint.
  // Margin and SBL fall back independently: one healthy report must not prevent
  // the other report from trying its fallback chain.
  const roc = isoToRocSlash(iso);
  const gregorian = String(iso || '').replace(/-/g, '/');
  const modern = (base, date) => `${base}?date=${encodeURIComponent(date)}&id=&response=json`;
  const marginUrls = [
    modern(TPEX_MARGIN_MODERN, roc),
    modern(TPEX_MARGIN_MODERN, gregorian),
    `${TPEX_MARGIN_LEGACY}?l=zh-tw&o=json&d=${encodeURIComponent(roc)}&s=0,asc`
  ];
  const sblUrls = [
    modern(TPEX_SBL_MODERN, gregorian),
    modern(TPEX_SBL_MODERN, roc),
    `${TPEX_SBL_LEGACY}?l=zh-tw&o=json&d=${encodeURIComponent(roc)}&s=0,asc,0`
  ];

  const [marginResult, sblResult] = await Promise.all([
    fetchTpexParsed(marginUrls, parseTpexMargin, iso),
    fetchTpexParsed(sblUrls, parseTpexSbl, iso)
  ]);
  if (!marginResult.rows.length && !sblResult.rows.length) {
    const details = [...marginResult.errors, ...sblResult.errors].filter(Boolean);
    if (details.length) throw new Error(`TPEx credit ${iso} failed: ${details.slice(0,3).join(' | ')}`);
    return [];
  }
  return mergeCreditRows(marginResult.rows, sblResult.rows);
}
...
async function existingCreditDates(market, limit = 550) {
  await ensureCreditTradingSchema();
  const sql = getSql();
  // A date is complete only when BOTH official report families reached DB.
  const rows = await sql.query(`
    SELECT trade_date::text AS d
    FROM credit_trading_daily
    WHERE market=$1
    GROUP BY trade_date
    HAVING COUNT(*) FILTER (WHERE margin_source IS NOT NULL) > 0
       AND COUNT(*) FILTER (WHERE sbl_source IS NOT NULL) > 0
...
// v2.6.5.16: reuse persisted price history as the official trading calendar so
// weekends/holidays never consume credit API calls during 250/500D catch-up.
...
async function runCreditTradingBackfill({ targetTradingDays = 500, maxNewDays = 4, maxRunMs = 42000 } = {}) {
  await ensureCreditTradingSchema();
  const started = Date.now();
  const target = Math.max(20, Math.min(500, Number(targetTradingDays) || 500));
  const perMarketBudget = Math.max(1, Math.min(10, Number(maxNewDays) || 4));
  const addedDates = { '上市': [], '上櫃': [] }, errors = [], markets = {};

  for (const market of ['上市','上櫃']) {
    if (Date.now() - started >= maxRunMs) break;
    const existing = await existingCreditDates(market, Math.max(550, target + 30));
    ...
    for (const iso of missing) {
      if (Date.now() - started >= maxRunMs) break;
      if (addedDates[market].length >= perMarketBudget || attempts >= maxAttempts) break;
      attempts++;
      try {
        const rows = await fetchDay(iso);
        if (!rows.length) {
          errors.push({ market, tradeDate: iso, error: 'official report returned 0 rows' });
          continue;
        }
        await upsertCreditTradingRows(rows);
        // Re-query completeness semantics locally: fetchDay merges margin + SBL;
        // only count it complete when both source families are represented.
        const hasMargin = rows.some(r => r.margin_source);
        const hasSbl = rows.some(r => r.sbl_source);
        if (hasMargin && hasSbl) {
          existing.add(iso);
          addedDates[market].push({ tradeDate: iso, rows: rows.length });
        } else {
          errors.push({ market, tradeDate: iso, error: `partial credit day: margin=${hasMargin}, sbl=${hasSbl}` });
        }
      } catch (e) {
  ...
  return {
    ...
    complete, targetTradingDays: target, source: 'credit_trading_backfill', markets,
...
const historyFillInFlight = new Map();

async function fillCreditHistoryForMarket(market, { targetDays = 20, maxRunMs = 26000, batchSize = 3 } = {}) {
  await ensureCreditTradingSchema();
  const existing = await existingCreditDates(market);
  if (existing.size >= targetDays) return { ok: true, market, complete: true, tradingDays: existing.size, addedDates: [] };

  if (historyFillInFlight.has(market)) return historyFillInFlight.get(market);
  const task = (async () => {
    const started = Date.now();
    const fetchDay = market === '上市' ? fetchTwseCreditDay : fetchTpexCreditDay;
    const anchorDate = await latestStoredDate(market) || taipeiTodayIso();
    const candidates = recentWeekdays(Math.max(targetDays + 28, 48), anchorDate).filter(d => !existing.has(d));
    const addedDates = [], errors = [];

    for (let i = 0; i < candidates.length && existing.size < targetDays; i += batchSize) {
      if (Date.now() - started >= maxRunMs) break;
      const batch = candidates.slice(i, i + batchSize);
      const results = await Promise.all(batch.map(async tradeDate => {
        try { return { tradeDate, rows: await fetchDay(tradeDate) }; }
        catch (e) { return { tradeDate, rows: [], error: String(e?.message || e) }; }
      }));
      for (const result of results) {
        if (result.rows?.length) {
          await upsertCreditTradingRows(result.rows);
          existing.add(result.tradeDate);
          addedDates.push({ tradeDate: result.tradeDate, rows: result.rows.length });
        } else if (result.error) {
          errors.push({ tradeDate: result.tradeDate, error: result.error });
        }
        if (existing.size >= targetDays) break;
      }
    }
    return {
      ok: existing.size > 0, market, complete: existing.size >= targetDays, tradingDays: existing.size,
      addedDates, errors: errors.slice(0, 8), elapsedMs: Date.now() - started
    };
  })();
  historyFillInFlight.set(market, task);
  try { return await task; }
  finally { historyFillInFlight.delete(market); }
}

async function creditBootstrapAllowed(market, cooldownMinutes = 3) {
  try {
    const sql = getSql();
    const source = SOURCE_BY_MARKET[market];
    if (!source) return true;
    const rows = await sql.query(`
      SELECT last_attempt_at,status
      FROM sync_status
      WHERE source=$1
      LIMIT 1
    `, [source]);
    if (!rows.length || !rows[0]?.last_attempt_at) return true;
    const ageMs = Date.now() - new Date(rows[0].last_attempt_at).getTime();
    return !Number.isFinite(ageMs) || ageMs >= Math.max(1, cooldownMinutes) * 60000;
  } catch {
    return true;
  }
}

// v2.6.5.13: DB-first remains the steady-state path, but `available` no longer
// means `history complete`.  v2.6.5.11 returned immediately after the first good
// day, which permanently left 5/10/20-day cards blank.  A lookup now fills the
// market history toward 20 real trading days whenever it is incomplete.  The
// fill is market-wide and persisted, so this cost is paid once and then reused.
async function ensureCreditTradingForStock(code, market = '', { targetDays = 20 } = {}) {
  const normalizedMarket = /上櫃|otc|tpex|two/i.test(String(market)) ? '上櫃'
    : /上市|twse|sii/i.test(String(market)) ? '上市' : '';
  const current = await readCreditTradingForStock(code, normalizedMarket || market);
  if (!normalizedMarket) return { ...current, storage: current.available ? 'db' : 'db_empty', bootstrap: { ok: false, skipped: true, reason: 'unknown_market' } };
  if (current.historyCount >= targetDays) return { ...current, storage: 'db' };

  let bootstrap;
  try { bootstrap = await fillCreditHistoryForMarket(normalizedMarket, { targetDays, maxRunMs: 26000, batchSize: normalizedMarket === '上櫃' ? 2 : 3 }); }
  catch (e) { bootstrap = { ok: false, market: normalizedMarket, preservedLastGood: true, error: String(e?.message || e) }; }
  const after = await readCreditTradingForStock(code, normalizedMarket);

  // If no row for this stock was found at all, do one latest-snapshot recovery.
  // This covers a brand-new DB even when the historical fill hit a transient
  // upstream timeout before reaching the latest published trading day.
  if (!after.available && await creditBootstrapAllowed(normalizedMarket, 2)) {
    try { await syncLatestMarket(normalizedMarket); } catch {}
    const recovered = await readCreditTradingForStock(code, normalizedMarket);
    return { ...recovered, storage: recovered.available ? 'db_bootstrap' : 'db_empty', bootstrap };
  }
  return { ...after, storage: after.available ? (after.historyCount >= targetDays ? 'db' : 'db_partial') : 'db_empty', bootstrap };
}

async function readCreditTradingHealth() {
  await ensureCreditTradingSchema();
  const sql = getSql();
  const rows = await sql.query(`
    WITH latest AS (
...
module.exports = {
  ensureCreditTradingSchema, upsertCreditTradingRows, runCreditTradingSync, runCreditTradingBackfill, fillCreditHistoryForMarket,
  readCreditTradingForStock, ensureCreditTradingForStock, readCreditTradingHealth, buildCreditSignal,
  _test: {
    num, lotsToShares, rawShares, ymdToIso, rocToIso, recentWeekdays,
    parseTwseMargin, parseTwseSbl, parseTpexMargin, parseTpexSbl, tpexPayloadRows, tpexPayloadDate, mergeCreditRows, creditPeriods, pctFromChange
  }
};
