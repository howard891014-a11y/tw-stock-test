...
// - Native technology industries get a safe fine-business fallback only when no finer tag exists.
...
function resolveCompanyBusinessTags(profile={}, options={}){
  ...
  if(nativeTech && fineTechTags.length===0 && industryInfo.fallbackTag){
    links.push({id:industryInfo.fallbackTag,importance:"related",origin:"industry-fallback"});
...
function summarizeProfiles(profiles=[]){
  ...
  for(const p of profiles){
    ...
    const fineTech=resolved.technologyTags.some(x=>x.resolution!=="fallback");
