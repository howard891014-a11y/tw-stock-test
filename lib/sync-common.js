...
const HEADERS = {
  'User-Agent': 'Mozilla/5.0 (StockZone/2.6.2.14; Taiwan market snapshot)',
...
async function upsertCompanyProfileRows(rows) {
  ...
  await sql.query(`
    WITH incoming AS (
      SELECT * FROM jsonb_to_recordset($1::jsonb) AS x(
        stock_code text, stock_name text, market text, industry text, report_date date, source text
      )
    )
    INSERT INTO market_company_profile
      (stock_code,stock_name,market,industry,report_date,source,updated_at)
    SELECT stock_code,stock_name,market,industry,report_date,source,NOW()
    FROM incoming
    ON CONFLICT (stock_code) DO UPDATE SET
