...
if(!html.includes('app.js?v=2.6.4.1'))errors.push('app.js cache-buster not updated to v2.6.4.0');
for(const token of ['data-fundflow-scope="electronics-product"','data-fundflow-browser-scope="electronics-product"','fundflowIsElectronicsProduct','fundflowScopeBucket(item)!==fundflowBrowserScope']) if(!html.includes(token)&&!app.includes(token)) errors.push(`missing v2.6.4.1 token ${token}`);
...
console.log(`Fundflow UI/upgrade validation PASS — ${ids.length} unique DOM ids, v2.6.4.1 controls + fan assertions present`);
