...
function renderBrokerRows(){
  ...
  host.innerHTML=rows.map((x,i)=>{
    ...
    return `<div class="broker-row" data-broker="${targetBrokerName(x.row).replace(/"/g,"&quot;")}" data-index="${i}">
      ...
      <strong>${targetFmt(targetPriceValue(x.latest))}</strong>
      <time>${String(d).slice(0,10)}</time>
    </div>`;
  }).join("");
  host.querySelectorAll(".broker-row").forEach((el,i)=>el.addEventListener("click",()=>renderMainTarget(rows[i])));
}
function renderTargetPlay(payload){
  targetRowsCache=normalizeTargetRows(payload);
  renderBrokerRows();
  renderMainTarget(pickMainTarget(targetRowsCache));
}
...
// target tabs: 外資 / 本土
document.querySelectorAll(".target-tabs button").forEach((btn,i)=>{
  btn.addEventListener("click",()=>{
    document.querySelectorAll(".target-tabs button").forEach(b=>b.classList.remove("active"));
    btn.classList.add("active");
    targetTypeFilter=i===0?"外資":"本土";
    renderBrokerRows();
  });
});
...
function addToList(type){
  ...
  if(!rows.some(x=>stockKey(x)===key)){rows.unshift(item);writeList(type,rows)}
...
$("favoriteBtn")?.addEventListener("click",()=>addToList("watchlist"));
