// StockZone v2.6.5.18 — A-plan Market-Topic Fund-Flow XY Engine v3.1.0 + Phase Transition Engine v1
...
const ENGINE_VERSION = 'xy-3.1.0-a-xv2-themeflow-business-weight';
...
function computeTagDetail(profiles,activityRows,tagId,{maxDates=10}={}){
  ...
  return {ok:true,engineVersion:ENGINE_VERSION,tagId:cleanTag,name:meta.name,parentName:meta.parentName||'',scope:meta.scope,asOf:latestPoint.date,trajectoryDays:enrichedTrajectory.length,trajectory:enrichedTrajectory,
    ...
    methodology:{impact:'公司貢獻＝移除該公司後，連同跨題材排名一起重新計算當日 XY；核心／重要／相關業務權重分別為 1.0／0.5／0.2。',factor:'X v2.1 先在題材內以業務關聯權重聚合法人淨資金流，再跨題材排名；信用交易最多只做 ±15 分籌碼品質修正。資料完整度主要進 confidence，只有法人覆蓋低於 20% 才把 X 拉向中性。',projection:'扇形是狀態＋速度＋歷史同狀態轉態分布的不確定範圍，不是未來保證。'}};
}
...
async function getFundflowSnapshot({days=10,force=false}={}){
  ...
  const value={ok:true,engineVersion:ENGINE_VERSION,asOf:allDates.at(-1),trajectoryDates:allDates.slice(-bounded),trajectoryDays:Math.min(bounded,allDates.length),
    ...
    methodology:{stockX:'個股法人流仍保留作診斷；正式題材 X 不再先把每檔股票排名後平均。',stockY:'10% 當日漲跌＋25% 3日報酬＋30% 5日報酬＋20% 20日報酬＋15% 近5日上漲持續性；價格因子採全市場百分位。',group:'X v2.1 先在每個題材內聚合法人淨流向：核心／重要／相關業務權重＝1.0／0.5／0.2，再把題材 1/5/10/20 日法人流、買超廣度、連買賣與三大法人一致度跨題材正規化；信用交易僅 ±15 分修正。一般資料完整度只影響 confidence，不再把正常 X 大幅壓回 50；座標仍用 3 交易日 EMA 平滑。Legacy 成交活動 X 保留為回測 baseline。',phase:'C＝族群同步確認度；E＝高檔過熱／衰竭度；Phase 另結合 ΔX/ΔY 與加速度並使用兩日狀態黏性。',projection:'Phase、ΔXY 與歷史同狀態轉態分布形成模型傾向；點開題材顯示不確定扇形。',caution:'X v2.1 是官方法人淨流向的題材相對強弱模型，信用交易是籌碼品質修正；不是逐筆成交資金流，也不是未來保證。'},
...
module.exports={ENGINE_VERSION,DEFAULT_TRAJECTORY_DAYS,ENGINE_HISTORY_DAYS,PHASES,percentileRanks,percentileRanksNullable,enrichFlowFeatures,scoreStocksForDate,summarizeTagItemsRaw,normalizeTagSummaries,aggregateTagDate,computeBusinessFlow,computeTagDetail,quadrant,buildTransitionCalibration,projectGroup,ensureBusinessFlowSchema,refreshBusinessFlowDaily,getFundflowSnapshot,getFundflowDetail,getFundflowBusinessBrowser,buildBusinessBrowserCatalog};
