...
function applyCorr(rows){
 ...
 return rows.map(row=>{
  ...
  if(fix.broker){r.broker=fix.broker;r.brokerType=fix.brokerType||inferredBrokerType(fix.broker,targetBrokerType(r));}
...
$("saveTargetEdit")?.addEventListener("click",()=>{
 ...
 c[id]={broker,target,brokerType:inferredBrokerType(broker,targetBrokerType(editingTarget))};writeCorr(c);
