// StockZone v2.6.5.19 — A-plan Market-Topic Fund-Flow XY Engine v3.2.0 + Phase Transition Engine v1
...
function computeTagDetail(profiles,activityRows,tagId,{maxDates=10}={}){
  ...
  return {ok:true,engineVersion:ENGINE_VERSION,tagId:cleanTag,name:meta.name,parentName:meta.parentName||'',scope:meta.scope,asOf:latestPoint.date,trajectoryDays:enrichedTrajectory.length,trajectory:enrichedTrajectory,
    ...
    methodology:{impact:'公司貢獻＝移除該公司後，連同跨題材排名一起重新計算當日 XY；核心／重要／相關基礎權重為 1.0／0.5／0.2，核心不稀釋，非核心跨題材總曝險上限 1.5。',factor:'X v2.2 先在題材內以業務關聯權重聚合法人淨資金流，再跨題材排名；信用交易最多只做 ±15 分籌碼品質修正。資料完整度主要進 confidence，只有法人覆蓋低於 20% 才把 X 拉向中性。',projection:'扇形是狀態＋速度＋歷史同狀態轉態分布的不確定範圍，不是未來保證。'}};
}
