...
function companyProfileRows(data, market, source) {
  ...
  return data.map(x => {
    ...
    const name = clean(first(x, ['公司簡稱','公司名稱','CompanyName','SecuritiesCompanyName','Name']));
    if (!name) return null;
    return {
      stock_code: code,
      stock_name: name,
      market,
      industry: clean(first(x, ['產業別','產業類別','Industry','IndustryCategory','IndustryName'])),
...
module.exports = {
  runPriceSync, runCompanyProfileSync, runTwseDisposalSync, runTpexDisposalSync, runMarketHistoryBackfill,
  _test: {
