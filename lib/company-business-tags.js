...
function listCompanyTags(symbol, options = {}) {
  const item = getCompany(symbol);
  if (!item) return [];
  const voteEligibleOnly = options.voteEligibleOnly !== false;
  return item.tags
    .map((x) => {
      const tag = getTag(x.id);
      if (!tag) return null;
      if (voteEligibleOnly && !isVoteEligible(tag)) return null;
      return Object.freeze({
        ...x,
        name: tag.name,
        parent: tag.parent,
...
function getCoverageSummary() {
  ...
  return Object.freeze({ companyCount, relationCount, representedTagCount });
}
