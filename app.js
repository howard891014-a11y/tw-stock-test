...
function fundflowTechStage(g){
  ...
  if(FUND_FLOW_TECH_DOWNSTREAM_IDS.has(tagId)||/伺服器|資料中心|電腦與週邊|網路與通訊設備|軟體與數位服務|電子製造與通路/.test(parent)||/資服|雲端|電子代工|AI PC|低軌衛星/.test(name))return "technology-downstream";
...
function fundflowScopeBucket(g){
  ...
  if(g.scope==="electronics-product")return "electronics-product";
  if(g.scope==="technology-fine")return fundflowTechStage(g);
  return "traditional";
}
...
function fundflowAxisTick(v){const n=Number(v)||0,abs=Math.abs(n);const d=abs>=10?0:abs>=1?1:2;return `${n>0?'+':''}${n.toFixed(d)}%`;}
...
function renderFundflowChart(){
  ...
  const xBound=fundflowNiceBound(xv,.5),yBound=fundflowNiceBound(yv,2),sx=x=>L+((Math.max(-xBound,Math.min(xBound,Number(x)||0))+xBound)/(2*xBound))*pw,sy=y=>T+((yBound-Math.max(-yBound,Math.min(yBound,Number(y)||0)))/(2*yBound))*ph;
  ...
  yTicks.forEach(v=>{svg.append(fundflowSvg("line",{x1:L,y1:sy(v),x2:L+pw,y2:sy(v),class:v===0?"fundflow-midline":"fundflow-gridline"}));svg.append(fundflowSvg("text",{x:L-10,y:sy(v)+4,class:"fundflow-axis-label","text-anchor":"end"},fundflowAxisTick(v)))});
  svg.append(fundflowSvg("text",{x:L+pw/2,y:H-8,class:"fundflow-axis-label","text-anchor":"middle"},"近5日法人淨買賣超 / 成交值 X (%) →"));svg.append(fundflowSvg("text",{x:15,y:T+ph/2,class:"fundflow-axis-label",transform:`rotate(-90 15 ${T+ph/2})`,"text-anchor":"middle"},"近5日題材價格漲跌幅 Y (%) →"));
...
function fundflowListHtml(groups,kind){
  ...
  return groups.slice(0,6).map(g=>{const leaders=(g.leaders||[]).slice(0,3).map(x=>x.name||x.code).filter(Boolean).join("、"),score=kind==="rising"?g.potentialScore:kind==="mainline"?g.mainlineScore:g.coolingScore,sel=g.tagId===fundflowSelectedTagId?" is-selected":"",proj=g.projection||{};return `<button type="button" class="fundflow-radar-item${sel}" data-fundflow-tag="${escNews(g.tagId)}"><b>${escNews(g.name||g.tagId)}</b><em>${fundflowFmt(score,0)}</em><small>${escNews(fundflowPhaseLabel(g))}｜X ${fundflowSignedPct(g.x)} / Y ${fundflowSignedPct(g.y)}｜C ${fundflowFmt(g.confirmation,0)} / E ${fundflowFmt(g.overheating,0)}｜3日 ΔX ${fundflowSigned(g.dx3)} / ΔY ${fundflowSigned(g.dy3)}${proj?.scenarios?.length?`｜A ${escNews(proj.scenarios[0]?.direction||"--")} ${fundflowFmt(proj.scenarios[0]?.confidence,0)}%${proj.scenarios[1]?` / B ${escNews(proj.scenarios[1]?.direction||"--")} ${fundflowFmt(proj.scenarios[1]?.confidence,0)}%`:""}`:proj.tendency?`｜模型 ${escNews(proj.tendency)} ${fundflowFmt(proj.confidence,0)}%`:""}${leaders?`｜${escNews(leaders)}`:""}</small></button>`}).join("");
}
function renderFundflowXy(){
  ...
  const note=$("fundflowMethodNote");if(note){const cal=data.calibration||{};note.textContent=`XY v4＝原始市場狀態：X 是近5日三大法人淨買賣超金額／成交值；Y 是題材成分股近5日價格報酬。0 為真正中線，不做百分位、不加信用修正、不用 EMA 改寫座標。C/E、信用交易、廣度與一致度只供 Phase / Future Path 解讀。實線＝已發生；點開顯示 A/B 兩條預測走廊。轉態樣本 ${Number(cal.historyDays||0)} 個交易日${cal.warmup?"（暖機期，信心會自動壓低）":""}。`}
...
function fundflowFactorHtml(factors,axis,priorFactors={},group={},priorPoint={}){
  const defs=axis==="x"?[["inst1","法人1日淨買賣超/成交值","觀察"],["inst5","法人5日淨買賣超/成交值","X座標"],["inst10","法人10日淨買賣超/成交值","觀察"],["inst20","法人20日淨買賣超/成交值","觀察"]]:[["changePct","題材1日漲跌","觀察"],["return3Pct","題材3日漲跌","觀察"],["return5Pct","題材5日漲跌","Y座標"],["return20Pct","題材20日漲跌","觀察"]];
  const rows=defs.map(([key,label,role])=>{const v=Number(factors?.[key])||0,pv=Number(priorFactors?.[key]),delta=Number.isFinite(pv)?v-pv:null,bar=Math.max(4,Math.min(96,50+(axis==="x"?v*10:v*5)));return `<div class="fundflow-factor-row"><label>${label} <small>${role}${delta===null?"":`｜3日變化 ${fundflowSignedPct(delta,2)}`}</small></label><strong>${fundflowSignedPct(v,2)}</strong><div class="fundflow-factor-bar"><i style="width:${bar}%"></i></div></div>`});
  if(axis==="x"){const flow=Math.max(0,Math.min(100,Number(group.flowBreadth??group.activityBreadth)||0)),concentration=Math.max(0,Math.min(100,Number(group.concentrationQuality)||0)),corr=Number(group.creditCorrection||0);rows.push(`<div class="fundflow-factor-row"><label>法人淨流入廣度 <small>Confirmation，不改 X</small></label><strong>${fundflowFmt(flow,0)}%</strong><div class="fundflow-factor-bar"><i style="width:${flow}%"></i></div></div>`);rows.push(`<div class="fundflow-factor-row"><label>資金集中品質 <small>Confirmation，不改 X</small></label><strong>${fundflowFmt(concentration,0)}</strong><div class="fundflow-factor-bar"><i style="width:${concentration}%"></i></div></div>`);rows.push(`<div class="fundflow-factor-row"><label>信用籌碼診斷 <small>不改 X</small></label><strong>${fundflowSigned(corr,1)}</strong><div class="fundflow-factor-bar"><i style="width:${Math.max(4,Math.min(96,50+corr/15*46))}%"></i></div></div>`)}
  else {const pb=Math.max(0,Math.min(100,Number(group.priceBreadth)||0)),ub=Math.max(0,Math.min(100,Number(group.upBreadth)||0));rows.push(`<div class="fundflow-factor-row"><label>5日上漲廣度 <small>Confirmation，不改 Y</small></label><strong>${fundflowFmt(pb,0)}%</strong><div class="fundflow-factor-bar"><i style="width:${pb}%"></i></div></div>`);rows.push(`<div class="fundflow-factor-row"><label>當日上漲廣度 <small>Confirmation，不改 Y</small></label><strong>${fundflowFmt(ub,0)}%</strong><div class="fundflow-factor-bar"><i style="width:${ub}%"></i></div></div>`)}
  return rows.join("");
}
function renderFundflowDetail(){
  ...
  setText("fundflowDetailX",fundflowSignedPct(d.latest?.x));setText("fundflowDetailY",fundflowSignedPct(d.latest?.y));setText("fundflowDetailMove",`C ${fundflowFmt(d.latest?.confirmation,0)} / E ${fundflowFmt(d.latest?.overheating,0)}`);setText("fundflowDetailStatus",`${d.latest?.phaseLabel||d.latest?.statusLabel||"--"}｜3日 ΔX ${fundflowSigned(d.latest?.dx3)} / ΔY ${fundflowSigned(d.latest?.dy3)}`);
