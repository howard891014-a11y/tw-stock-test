// StockZone v2.6.5.12 — official credit-trading history layer.
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      const r = await fetch(url, {
        ...
        redirect: 'follow',
        headers: {
          'User-Agent': 'Mozilla/5.0 (StockZone/2.6.5.12; Taiwan credit trading)',
...
async function existingCreditDates(market) {
  await ensureCreditTradingSchema();
  const sql = getSql();
  // A calendar day is only considered complete when BOTH official report families
  // reached DB.  Otherwise a temporary SBL-only (or margin-only) success would
  // permanently suppress retries for the missing half of the credit cards.
...
// v2.6.5.12: DB-first remains the steady-state path, but `available` no longer
...
async function readCreditTradingHealth() {
  ...
  const rows = await sql.query(`
    SELECT market, COUNT(*)::int AS rows, COUNT(DISTINCT trade_date)::int AS trading_days,
      MIN(trade_date)::text AS min_date, MAX(trade_date)::text AS max_date, MAX(updated_at)::text AS updated_at
    FROM credit_trading_daily
    GROUP BY market
    ORDER BY market
  `);
