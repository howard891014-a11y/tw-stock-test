...
async function runMarketHistoryBackfill({
  ...
  try{
    ...
    for(const [index,market] of ['上市','上櫃'].entries()){
      ...
      try{
        if(market==='上市'&&rows.length<300)throw new Error(`TWSE 最新行情有效筆數異常：${rows.length}`);
        if(market==='上櫃'&&rows.length<200)throw new Error(`TPEx 最新行情有效筆數異常：${rows.length}`);
        assertMarketHistoryCoverage(rows,market);
        await upsertPriceAndHistoryRows(rows);
        bootstrap[market]={ok:true,rows:rows.length,tradeDate:[...new Set(rows.map(x=>x.trade_date))][0]||null,upstream:result.value?.upstream||null};
      }catch(e){
    ...
    while(scanned<scanCalendarDays&&completeDates.length<targetTradingDays){
      ...
      for(let i=0;i<tasks.length;i++){
        ...
        try{
          ...
          day[market]={ok:true,rows:data.rows.length,upstream:data.upstream};
        }catch(e){
          sourceFailure=true;
    ...
    return{
      ...
      elapsedMs:Date.now()-startedAt,added,bootstrap,latestByMarket,
