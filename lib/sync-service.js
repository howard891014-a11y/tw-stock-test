...
const { refreshBusinessFlowDaily, ENGINE_HISTORY_DAYS } = require('./fundflow-xy');
...
async function runMarketHistoryBackfill({
  ...
  try{
    ...
    const existing=await sql.query(`
      ...
      WHERE market IN ('上市','上櫃')
        AND trade_date >= CURRENT_DATE - INTERVAL '900 days'
    ...
    return{
      ...
      nextAction:done?null:'背景 backfill 會從 DB 既有進度續補；也可再次執行 action=market-backfill 加速'
    };
  }catch(e){
...
async function runPriceSync({ cronSchedule = '' } = {}) {
  ...
  try {
    ...
    try{businessFlow=await refreshBusinessFlowDaily({trajectoryDays:ENGINE_HISTORY_DAYS})}
