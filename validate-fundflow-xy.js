...
// v2.6.4.1 glass substrate 7-company regression:
// exact-symbol + market-topic overlays must keep 雷科 / 鈦昇 / 創新服務 in the same denominator.
{
  const gsNames=[['1802','台玻'],['3481','群創'],['8064','東捷'],['6405','悅城'],['6207','雷科'],['8027','鈦昇'],['7828','創新服務']];
  const gsProfiles=gsNames.map(([stock_code,stock_name])=>({stock_code,stock_name,market:'上櫃',industry_code:'24',industry:'半導體業',auto_business_tags:stock_name==='悅城'?['glass_substrate']:[]}));
  const gsRows=[];
  for(const trade_date of ['2026-09-23','2026-09-24'])for(const [stock_code,stock_name] of gsNames)gsRows.push({trade_date,stock_code,stock_name,market:'上櫃',trade_value:100,change_pct:1,value_ratio_20:1.2,value_trend_5_15:1.1,positive_days_5:3,return_3_pct:2,return_5_pct:3,return_20_pct:4});
  ...
  assert.equal(gs.memberCount,7,'glass substrate denominator must include 7 mapped companies');
  assert.equal(gs.validCount,7,'glass substrate valid group must include all 7 activity-ready companies');
}
