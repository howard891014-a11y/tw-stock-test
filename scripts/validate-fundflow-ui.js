...
for(const token of ['fundflow-forecast-corridor','fundflow-future-path','fundflow-future-label','fundflow-controls-box .fundflow-phase-row','.fundflow-quad-legend{display:none!important}']) if(!app.includes(token)&&!css.includes(token)&&!html.includes(token)) errors.push(`missing UI token ${token}`);
...
if(!html.includes('.fundflow-forecast-corridor{'))errors.push('forecast corridor CSS missing');
