// StockZone v2.6.2.33-C5 — official MOPS business-profile enrichment
//
// Purpose:
...
async function runBusinessEnrichment({limit=30,delayMs=120,maxRunMs=42000,retry=false}={}){
  ...
  // Two cleanup queues:
  // 1) native-tech industry-baseline profiles that still need a real fine tag;
  // 2) fully unmapped profiles (typically special/management-stock industry codes).
  const pending=beforeRows.filter(p=>{
    const r=resolveCompanyBusinessTags(profileForResolver(p));
    return r.resolution==='industry-baseline'||r.resolution==='none';
  });
  ...
  for(let i=0;i<picked.length;i++){
    ...
    try{
      ...
      const tags=classifyBusinessText(detail.mainBusiness,{industryCode:p.industry_code,code:p.stock_code,allowBroad:currentResolution==='none'});
  ...
  return {
    ...
    remainingUnprocessed:afterRows.filter(p=>{
      const r=resolveCompanyBusinessTags(profileForResolver(p));
      return (r.resolution==='industry-baseline'||r.resolution==='none')&&!p.business_enrich_checked_at;
    }).length,
...
module.exports={RULES,BROAD_RULES,CODE_OVERRIDES,classifyBusinessText,classifyBroadBusinessText,fetchMopsProfile,runBusinessEnrichment,readPendingBusinessEnrichment,readUnclassifiedProfiles};
