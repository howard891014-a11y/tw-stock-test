...
const TOPIC_EVIDENCE_RULES=Object.freeze({
  ...
  drone:{strong:[/無人機/iu,/\bUAV\b/iu,/\bdrone\b/iu],support:[/無人載具|飛控|航太|軍工/iu]},
  heavy_electrical:{strong:[/重電/iu],support:[/變壓器/iu,/GIS/iu,/配電盤/iu,/開關設備/iu,/輸配電/iu,/電網/iu,/電力設備/iu]},
  robot:{strong:[/人形機器人/iu,/協作型機器人/iu,/工業機器人/iu,/機械手臂/iu],support:[/機器人/iu,/robot/iu,/自動化/iu]},
  ...
  thermal:{strong:[/液冷/iu,/水冷/iu,/cold\s*plate/iu,/冷板/iu,/\bCDU\b/iu,/散熱模組/iu,/均熱板/iu,/vapor\s*chamber/iu],support:[/散熱/iu,/伺服器|server|資料中心/iu]},
  power_semiconductor:{strong:[/\bMOSFET\b/iu,/\bIGBT\b/iu,/碳化矽|\bSiC\b/iu,/氮化鎵|\bGaN\b/iu],support:[/功率半導體|功率元件/iu]},
...
function detectMarketTopicEvidence(text,{minScore=0}={}){
  ...
  for(const [id,rule] of Object.entries(TOPIC_EVIDENCE_RULES)){
    ...
    if(!strong.length){
      // "重電" is often described through a cluster of products rather than the literal market nickname.
  ...
  for(const def of listMarketDefinitions({activeOnly:true})){
    ...
    const candidates=uniq([def.name,...(def.aliases||[])]).filter(x=>{
      const c=cleanEvidenceText(x);return c.length>=3&&!['科技','金融','傳產','其他產業','機械','化工','塑化','綠能'].includes(c);
    });
...
module.exports=Object.freeze({
  version:'1.1.0',TOPIC_SPECS,TOPIC_EVIDENCE_RULES,listMarketDefinitions,marketDefinitionById,marketDefinitionForSource,
