...
const TAGS = Object.freeze([
  ...
  tag("bmc", "BMC", "server", { voteEligible: true, aliases: ["基板管理控制器", "基板管理控制系統", "Baseboard Management Controller"] }),
  ...
  tag("oled", "OLED", "display", { voteEligible: true, aliases: ["OLED面板", "PMOLED", "AMOLED", "Micro-OLED"] }),
  ...
  tag("pneumatic_component", "氣動元件", "automation", { voteEligible: true, aliases: ["氣動控制元件", "Pneumatic Component", "Pneumatics"] }),
  ...
  tag("electronic_distribution_business", "電子通路", "electronics_services", { voteEligible: true, aliases: ["電子零組件通路"] }),
  tag("automation_machine", "自動化機台", "automation", { voteEligible: true, aliases: ["自動化設備機台"] }),

  // ---------- v2.6.2.33 official-chain product/process fine tags ----------
  // These tags replace native-tech baseline-only classification with concrete products,
  // processes, equipment or service roles backed by official industry-chain membership.
  tag("led_driver_ic", "LED驅動IC", "ic_design", { voteEligible: true, aliases: ["LED Driver IC"] }),
  tag("motor_driver_ic", "馬達驅動IC", "ic_design", { voteEligible: true, aliases: ["Motor Driver IC", "BLDC Driver", "馬達控制IC"] }),
  tag("optical_comm_ic", "光通訊IC", "ic_design", { voteEligible: true, aliases: ["Optical Communication IC"] }),
  tag("light_source_management_ic", "光源管理IC", "ic_design", { voteEligible: true, aliases: ["Light Source Management IC"] }),
  tag("consumer_ic", "消費性IC", "ic_design", { voteEligible: true, aliases: ["Consumer IC"] }),
  tag("memory_ic", "記憶體IC", "ic_design", { voteEligible: true, aliases: ["Memory IC"] }),
  tag("hdd_controller_ic", "磁碟儲存控制IC", "ic_design", { voteEligible: true, aliases: ["HDD Controller IC"] }),
  tag("io_interface_ic", "輸出入介面IC", "ic_design", { voteEligible: true, aliases: ["I/O Interface IC"] }),
  tag("display_controller_ic", "顯示控制IC", "ic_design", { voteEligible: true, aliases: ["Display Controller IC"] }),
  tag("optical_storage_controller_ic", "光儲存控制IC", "ic_design", { voteEligible: true }),
  tag("image_sensor_ic", "影像感測IC", "ic_design", { voteEligible: true, aliases: ["Image Sensor IC"] }),

  tag("wafer_manufacturing", "晶圓製造", "wafer_process", { voteEligible: true, aliases: ["IC/晶圓製造"] }),
  tag("discrete_semiconductor", "分離式半導體", "wafer_process", { voteEligible: true, aliases: ["Discrete Semiconductor", "二極體"] }),
  tag("semiconductor_process_test_equipment", "半導體製程／檢測設備", "semiconductor_equipment", { voteEligible: true, aliases: ["半導體製程設備", "半導體檢測設備"] }),
  tag("semiconductor_chemicals_materials", "半導體化學材料", "semiconductor_material", { voteEligible: true, aliases: ["半導體化學品"] }),
  tag("packaging_test_equipment", "封裝測試設備", "semiconductor_equipment", { voteEligible: true, aliases: ["封測設備"] }),
  tag("ic_substrate", "IC基板", "advanced_packaging", { voteEligible: true, aliases: ["封裝基板"] }),
  tag("leadframe", "導線架", "advanced_packaging", { voteEligible: true, aliases: ["Leadframe"] }),
  tag("ic_module", "IC模組", "advanced_packaging", { voteEligible: true }),
  tag("ic_distribution", "IC通路", "electronics_services", { voteEligible: true, aliases: ["IC經銷"] }),

  tag("display_chemicals_materials", "顯示器化學材料", "display", { voteEligible: true, aliases: ["面板化學材料"] }),
  tag("ito_substrate", "ITO導電玻璃／基板", "display", { voteEligible: true, aliases: ["ITO基板", "ITO導電玻璃"] }),
  tag("backlight_source", "背光光源", "display", { voteEligible: true }),
  tag("display_frame", "顯示器機構件", "display", { voteEligible: true, aliases: ["面板機構件"] }),
  tag("optical_film", "光學膜／稜鏡片", "display", { voteEligible: true, aliases: ["光學膜", "稜鏡片"] }),
  tag("backlight_module", "背光模組", "display", { voteEligible: true }),
  tag("display_module", "顯示器模組", "display", { voteEligible: true, aliases: ["面板模組"] }),
  tag("display_process_test_equipment", "顯示器製程／檢測設備", "display", { voteEligible: true, aliases: ["面板設備"] }),
  tag("led_epitaxy", "LED磊晶／光源", "optical", { voteEligible: true, aliases: ["LED磊晶", "LED光源"] }),
  tag("led_package_module", "LED封裝／模組", "optical", { voteEligible: true, aliases: ["LED封裝", "LED模組"] }),
  tag("solar_cell", "太陽能電池", "optical", { voteEligible: true }),
  tag("solar_module", "太陽能模組", "optical", { voteEligible: true }),

  tag("chipset", "晶片組", "computing", { voteEligible: true, aliases: ["Chipset"] }),
  tag("motherboard", "主機板", "computing", { voteEligible: true, aliases: ["Motherboard"] }),
  tag("computer_chassis", "電腦機殼／機構件", "computing", { voteEligible: true, aliases: ["電腦機殼"] }),
  tag("battery_module", "電池／電池模組", "power_electronics", { voteEligible: true, aliases: ["Battery Pack", "電池模組"] }),
  tag("io_interface_card", "輸出入模組／介面卡", "computing", { voteEligible: true, aliases: ["I/O介面卡"] }),
  tag("hard_disk_drive", "硬碟機", "storage", { voteEligible: true, aliases: ["HDD"] }),
  tag("optical_drive", "光碟機", "storage", { voteEligible: true }),
  tag("flash_storage_device", "隨身碟／記憶卡", "storage", { voteEligible: true, aliases: ["隨身碟", "記憶卡"] }),
  tag("video_capture_card", "視訊擷取／多功能視訊卡", "computing", { voteEligible: true, aliases: ["視訊卡"] }),
  tag("optical_disc", "光碟片", "storage", { voteEligible: true }),
  tag("hinge", "機構樞紐／轉軸", "computing", { voteEligible: true, aliases: ["Hinge", "轉軸"] }),
  tag("precision_mold", "精密模具", "computing", { voteEligible: true, aliases: ["金屬塑膠模具"] }),
  tag("notebook_pc", "筆記型電腦", "computing", { voteEligible: true, aliases: ["Notebook", "Laptop"] }),
  tag("desktop_pc", "桌上型電腦", "computing", { voteEligible: true, aliases: ["Desktop PC"] }),
  tag("thin_client", "精簡型電腦", "computing", { voteEligible: true, aliases: ["Thin Client"] }),
  tag("office_imaging_equipment", "印表／掃描／投影設備", "computing", { voteEligible: true, aliases: ["印表機", "掃瞄器", "投影機"] }),
  tag("server_system", "伺服器系統", "server", { voteEligible: true, aliases: ["Server System"] }),

  tag("wired_communication_equipment", "有線通訊設備", "networking", { voteEligible: true, aliases: ["Wired Communication Equipment"] }),
  tag("filter_oscillator", "濾波器／振盪器", "passive", { voteEligible: true, aliases: ["濾波器", "振盪器"] }),
  tag("software_distribution", "軟體通路／經銷", "software", { voteEligible: true, aliases: ["軟體經銷", "軟體代理"] }),


  // ---------- v2.6.2.33-C2 second-pass fine tags ----------
  // Added for MOPS main-business descriptions that were too specific for the first-pass matcher.
  tag("keyboard_input_device", "鍵盤／輸入裝置", "computing", { voteEligible: true, aliases: ["鍵盤", "滑鼠", "Keyboard", "Mouse"] }),
  tag("consumer_electronics", "消費性電子", "computing", { voteEligible: true, aliases: ["影音電子", "智慧裝置", "Consumer Electronics"] }),
  tag("audio_component", "音訊／揚聲器元件", "electronics_services", { voteEligible: true, aliases: ["喇叭", "揚聲器", "音響", "Speaker"] }),
  tag("test_measurement_instrument", "電子測試量測儀器", "automation", { voteEligible: true, aliases: ["示波器", "電子負載", "量測儀器", "Test & Measurement"] }),
  tag("semiconductor_facility_engineering", "半導體廠務／整廠工程", "semiconductor_equipment", { voteEligible: true, aliases: ["無塵室工程", "高科技廠房", "廠務工程"] }),
  tag("holographic_optical_material", "雷射全像／防偽光學材料", "optical", { voteEligible: true, aliases: ["雷射全像膜", "全像材料", "Holographic Film"] }),
  tag("optocoupler", "光耦合器", "optical", { voteEligible: true, aliases: ["光耦", "Photocoupler"] }),
  tag("relay_sensor", "繼電器／磁簧感測元件", "passive", { voteEligible: true, aliases: ["繼電器", "磁簧繼電器", "Reed Relay"] }),
  tag("precision_metal_components", "精密金屬／沖壓零組件", "electronics_services", { voteEligible: true, aliases: ["精密金屬零件", "沖壓零件", "冷鍛零件"] }),
  tag("card_reader", "讀卡機／RFID讀取設備", "computing", { voteEligible: true, aliases: ["讀卡機", "讀卡器", "RFID Reader"] }),
  tag("pos_payment_terminal", "POS／支付終端", "computing", { voteEligible: true, aliases: ["POS", "銷售點終端機", "支付終端"] }),
  tag("hdd_component", "硬碟／儲存零組件", "storage", { voteEligible: true, aliases: ["硬碟零組件", "HDD零件"] }),
  tag("antenna_rf", "天線／射頻元件", "networking", { voteEligible: true, aliases: ["天線", "射頻元件", "RF"] }),
  tag("rf_testing_certification", "射頻／EMC測試認證", "networking", { voteEligible: true, aliases: ["SAR測試", "EMC測試", "測試認證"] }),
  tag("cover_glass", "光電／顯示器玻璃", "display", { voteEligible: true, aliases: ["Cover Glass", "光電玻璃", "顯示器玻璃"] }),
  tag("acoustic_component", "聲學／蜂鳴器元件", "electronics_services", { voteEligible: true, aliases: ["蜂鳴器", "聲學元件", "Buzzer"] }),
  tag("automotive_electronics", "車用電子／電裝", "electronics_services", { voteEligible: true, aliases: ["車用電子", "車用電裝"] }),
  tag("automotive_sensor", "車用感測器", "electronics_services", { voteEligible: true, aliases: ["TPMS", "胎壓感測", "車用感測"] }),
  tag("pi_film", "PI／聚醯亞胺薄膜", "electronic_components_manufacturing", { voteEligible: true, aliases: ["PI膜", "Polyimide Film"] }),
  tag("online_game", "線上遊戲", "software", { voteEligible: true, aliases: ["Online Game", "遊戲軟體"] }),
  tag("ecommerce_platform", "電子商務平台", "software", { voteEligible: true, aliases: ["電商平台", "網路購物"] }),
  tag("digital_platform", "數位／網路平台", "software", { voteEligible: true, aliases: ["網路平台", "媒合平台"] }),
  tag("payment_platform", "電子支付／支付平台", "software", { voteEligible: true, aliases: ["第三方支付", "行動支付", "電子支付"] }),
  tag("digital_identity_antifraud", "數位身分／反詐服務", "software", { voteEligible: true, aliases: ["防詐", "反詐", "身分驗證", "來電辨識"] }),
  tag("iot_solution", "IoT／智慧聯網方案", "networking", { voteEligible: true, aliases: ["IoT", "AIoT", "物聯網"] }),
  tag("smart_meter_energy_management", "智慧電表／能源管理", "power_electronics", { voteEligible: true, aliases: ["智慧電表", "電能管理", "能源管理系統"] }),
  tag("led_lighting", "LED照明", "optical", { voteEligible: true, aliases: ["LED燈具", "LED Lighting"] }),
  tag("battery_material", "電池材料", "power_electronics", { voteEligible: true, aliases: ["正極材料", "陰極材料", "磷酸鐵鋰"] }),
  tag("semiconductor_analysis_service", "半導體分析／可靠度服務", "semiconductor_test", { voteEligible: true, aliases: ["故障分析", "可靠度分析", "材料分析"] }),
  tag("e_paper", "電子紙／E Ink", "display", { voteEligible: true, aliases: ["電子紙", "E Ink", "E-paper"] }),
  tag("pcba_ems", "PCBA／SMT組裝", "electronics_services", { voteEligible: true, aliases: ["PCBA", "SMT組裝", "電路板組裝"] }),
  tag("automatic_data_capture", "條碼／AIDC自動資料收集", "computing", { voteEligible: true, aliases: ["AIDC", "條碼掃描器", "自動資料收集"] }),
  tag("it_distribution", "資訊產品通路", "electronics_services", { voteEligible: true, aliases: ["電腦通路", "科技產品通路", "IT通路"] }),
  tag("display_device", "顯示器／LCD TV", "display", { voteEligible: true, aliases: ["液晶顯示器", "LCD TV", "顯示器產品"] }),
  tag("vacuum_coating_service", "真空鍍膜／濺鍍代工", "electronics_services", { voteEligible: true, aliases: ["真空鍍膜", "真空蒸鍍", "Sputter", "Vacuum Coating"] }),
  tag("keypad_mechanical_component", "按鍵／機構整合元件", "electronic_components_manufacturing", { voteEligible: true, aliases: ["按鍵", "按鍵飾品", "機構整合元件"] }),
  tag("infrared_thermal_sensor", "紅外線／熱感測器", "optical", { voteEligible: true, aliases: ["紅外線偵測器", "紅外線感測器", "熱感測器"] }),
  tag("electromechanical_switch", "電控開關／插座", "electronic_components_manufacturing", { voteEligible: true, aliases: ["電控裝置", "電源開關", "機電開關"] }),
  tag("solar_system_engineering", "太陽能系統／電廠工程", "optical", { voteEligible: true, aliases: ["太陽能光電工程", "太陽能電廠", "Solar EPC"] }),
  tag("solar_conductive_paste", "太陽能導電漿", "optical", { voteEligible: true, aliases: ["導電漿", "太陽能漿料"] }),
  tag("ceramic_substrate", "陶瓷基板", "electronic_components_manufacturing", { voteEligible: true, aliases: ["氧化鋁陶瓷基板", "氮化鋁基板", "DBC基板"] }),
  tag("capacitor_foil", "電容器鋁箔／化成箔", "passive", { voteEligible: true, aliases: ["化成鋁箔", "腐蝕鋁箔", "電解電容鋁箔"] }),
  tag("optical_filter_coating", "光學濾光片／鍍膜", "optical", { voteEligible: true, aliases: ["薄膜濾光片", "光學鍍膜", "Optical Coating"] }),
  tag("sip_module_packaging", "SiP／系統模組封裝", "advanced_packaging", { voteEligible: true, aliases: ["SiP", "系統模組封裝", "模組封裝"] }),
  tag("screen_printing_mesh", "精密網版／網印材料", "electronics_services", { voteEligible: true, aliases: ["精密網版", "網版", "網印耗材"] }),
  tag("it_service", "資訊服務／系統維運", "software", { voteEligible: true, aliases: ["資訊系統維運", "IT Service"] }),
  tag("consumer_electronics_retail", "消費電子／家電通路", "electronics_services", { voteEligible: true, aliases: ["家電通路", "3C通路", "消費電子通路"] }),

  // ---------- v2.6.2.33-C4 final-tail exact-business fine tags ----------
  // Added for the last 26 MOPS unresolved companies. These are concrete product/service
  // labels, not broad technology fallbacks.
  tag("biomedical_health", "生醫", "tech", { voteEligible: true, aliases: ["醫療器材", "數位健康", "智慧醫療", "生醫產品"] }),
  tag("solder_material", "電子焊接材料", "pcb", { voteEligible: true, aliases: ["焊錫條", "焊錫絲", "錫球", "錫膏", "助焊劑"] }),
  tag("functional_electronic_material", "電子功能材料", "electronics_services", { voteEligible: true, aliases: ["導電材料", "EMI材料", "絕緣材料", "電子機能材料"] }),
  tag("precision_metal_mask", "精密金屬遮罩", "display", { voteEligible: true, aliases: ["金屬遮罩", "蒸鍍遮罩", "Metal Mask"] }),
  tag("emc_protection_component", "EMC／線路保護元件", "passive", { voteEligible: true, aliases: ["EMC元件", "線路保護元件", "ESD保護元件"] }),
  tag("rfid_tag", "RFID電子標籤", "networking", { voteEligible: true, aliases: ["RFID標籤", "RFID Tag", "電子標籤"] }),
  tag("digital_remittance", "跨境匯兌／數位匯款", "software", { voteEligible: true, aliases: ["移工匯兌", "跨境匯款", "數位匯款"] }),
  tag("renewable_energy_equipment", "再生能源設備／電力系統", "power_electronics", { voteEligible: true, aliases: ["再生能源設備", "發電設備", "電力系統"] }),
  tag("pcb_tooling", "PCB鑽針／切削工具", "pcb", { voteEligible: true, aliases: ["PCB鑽針", "微型鑽針", "PCB銑刀"] }),
  tag("ferrite_magnetic_component", "鐵氧體／磁性元件", "passive", { voteEligible: true, aliases: ["鐵氧體", "磁鐵芯", "磁性元件"] }),
  tag("image_sensor_module", "影像感測模組", "imaging", { voteEligible: true, aliases: ["CIS模組", "CMOS感測模組", "線性感測模組"] }),
  tag("surface_treatment", "精密表面處理／電鍍", "electronics_services", { voteEligible: true, aliases: ["表面處理", "精密電鍍", "電鍍代工"] }),
  tag("digital_media_streaming", "OTT／數位影音", "software", { voteEligible: true, aliases: ["OTT", "數位影音", "串流影音", "數位內容"] }),
  tag("water_treatment_engineering", "水處理／環保工程", "green_energy", { voteEligible: true, aliases: ["水處理", "污水處理", "廢水處理", "環保工程"] }),
  ...
  tag("motorcycle", "機車／零組件", null, { kind: "sector", resolution: "coarse", voteEligible: true, aliases: ["機車", "機車零組件", "摩托車", "摩托車零組件"] }),
