// StockZone v2.6.5.16 — persisted official institutional-flow history layer with 250/500D research backfill.
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      const r = await fetch(url, {
        ...
        redirect: 'follow',
        headers: {
          'User-Agent': 'Mozilla/5.0 (StockZone/2.6.5.16; Taiwan institutional flow)',
...
async function runInstitutionalBackfill({ targetTradingDays = 500, maxNewDays = 4, maxRunMs = 42000 } = {}) {
  await ensureInstitutionalHistorySchema();
  const started = Date.now();
  const target = Math.max(20, Math.min(500, Number(targetTradingDays) || 500));
  const perMarketBudget = Math.max(1, Math.min(12, Number(maxNewDays) || 4));
