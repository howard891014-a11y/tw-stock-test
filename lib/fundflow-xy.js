// StockZone v2.6.2.28 — Business Fund-Flow XY Engine v2 + Phase Transition Engine v1
...
function buildBusinessBrowserCatalog(profiles=[],snapshot={}){
  ...
  return {
    ...
    counts:{
      totalDefinitions:items.length,technologyFineDefinitions:count(x=>x.scope==='technology-fine'),technologyFallbackDefinitions:count(x=>x.scope==='technology-fallback'),
      traditionalDefinitions:count(x=>x.scope==='traditional-coarse'),representedDefinitions:count(x=>x.represented),withXY:count(x=>x.xyEligible),withoutXY:count(x=>!x.xyEligible),
...
function classifyScope(link){if(link?.technology)return link?.resolution==='fallback'?'technology-fallback':'technology-fine';return 'traditional-coarse';}
function buildProfileTagMap(profiles){
  ...
  for(const p of profiles||[]){
    ...
    const resolved=resolveCompanyBusinessTags({stock_code:code,stock_name:p.stock_name,name:p.stock_name,market:p.market,industry_code:p.industry_code,industry:p.industry});
...
function scopeCounts(groups){const out={'technology-fine':0,'technology-fallback':0,'traditional-coarse':0};for(const g of groups)out[g.scope]=(out[g.scope]||0)+1;return out;}
