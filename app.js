...
function renderFundflowChart(){
  ...
  groups.slice().reverse().forEach(g=>{
    ...
    const cx=sx(g.x),cy=sy(g.y),projection=g.projection||null;
    // E' — all visible groups get only a short dashed center tendency before selection.
    if(projection?.points?.length){
      const projPoints=selectedNow?projection.points:(projection.points.filter(p=>Number(p.horizon)<=3));
      const linePoints=[`${cx},${cy}`,...projPoints.map(p=>`${sx(p.x)},${sy(p.y)}`)].join(" ");
      if(selectedNow&&fundflowDetailData?.tagId===g.tagId){
        const fanPts=projection.points.slice(0,3),upper=[`${cx},${cy}`,...fanPts.map(p=>`${sx(p.highX)},${sy(p.highY)}`)],lower=[...fanPts].reverse().map(p=>`${sx(p.lowX)},${sy(p.lowY)}`),poly=[...upper,...lower].join(" ");
        svg.append(fundflowSvg("polygon",{points:poly,class:"fundflow-projection-fan",fill:color,stroke:color,"aria-hidden":"true"}));
      }
      svg.append(fundflowSvg("polyline",{points:linePoints,class:`fundflow-projection${selectedNow?" is-selected":""}${dim?" is-dim":""}`,stroke:color,"aria-label":`${g.name} 模型轉態傾向：${projection.tendency||projection.direction||"--"}`}));
      if(selectedNow){const end=projPoints.at(-1);if(end)svg.append(fundflowSvg("circle",{cx:sx(end.x),cy:sy(end.y),r:4.5,fill:"#fff",stroke:color,class:"fundflow-projection-end"}))}
    }
...
function fundflowListHtml(groups,kind){
  ...
  return groups.slice(0,6).map(g=>{const leaders=(g.leaders||[]).slice(0,3).map(x=>x.name||x.code).filter(Boolean).join("、"),score=kind==="rising"?g.potentialScore:kind==="mainline"?g.mainlineScore:g.coolingScore,sel=g.tagId===fundflowSelectedTagId?" is-selected":"",proj=g.projection||{};return `<button type="button" class="fundflow-radar-item${sel}" data-fundflow-tag="${escNews(g.tagId)}"><b>${escNews(g.name||g.tagId)}</b><em>${fundflowFmt(score,0)}</em><small>${escNews(fundflowPhaseLabel(g))}｜X ${fundflowFmt(g.x)} / Y ${fundflowFmt(g.y)}｜C ${fundflowFmt(g.confirmation,0)} / E ${fundflowFmt(g.overheating,0)}｜3日 ΔX ${fundflowSigned(g.dx3)} / ΔY ${fundflowSigned(g.dy3)}${proj.tendency?`｜虛線 ${escNews(proj.tendency)} ${fundflowFmt(proj.confidence,0)}%`:""}${leaders?`｜${escNews(leaders)}`:""}</small></button>`}).join("");
}
function renderFundflowXy(){
  const data=fundflowXyData;if(!data)return renderFundflowChart();setText("fundflowAsOf",data.asOf?`${String(data.asOf).replaceAll("-","/")}｜${data.trajectoryDays}日`:"--");setText("fundflowEngineBadge",String(data.engineVersion||"XY v2").replace("xy-","XY "));
  const groups=fundflowEligibleGroups(),rising=groups.filter(g=>["germination","potential"].includes(g.phaseState)&&Number(fundflowProjectionPoint(g,5)?.dy||0)>0).sort((a,b)=>Number(b.potentialScore)-Number(a.potentialScore)),mainline=groups.filter(g=>g.phaseState==="mainline").sort((a,b)=>Number(b.mainlineScore)-Number(a.mainlineScore)),cooling=groups.filter(g=>["overheating","cooling"].includes(g.phaseState)).sort((a,b)=>Number(b.coolingScore)-Number(a.coolingScore));
  ...
  const note=$("fundflowMethodNote");if(note){const cal=data.calibration||{};note.textContent=`X 已改為純資金活動度；Y 為價格強度並加入 3 日報酬，座標以 3 日 EMA 平滑。實線＝已發生；虛線＝模型轉態傾向；點開才顯示不確定扇形。轉態樣本 ${Number(cal.historyDays||0)} 個交易日${cal.warmup?"（暖機期，信心會自動壓低）":""}。`}
...
function renderFundflowDetail(){
  ...
  const proj=d.projection||{},pp=fundflowProjectionPoint({projection:proj},5);setText("fundflowDetailProjection",proj.tendency?`${proj.tendency} ${fundflowFmt(proj.confidence,0)}%`:"--");setText("fundflowDetailProjectionNote",pp?`5日中心傾向 X ${fundflowFmt(pp.x)} / Y ${fundflowFmt(pp.y)}｜方向 ${proj.direction||"--"}`:"等待模型傾向");
  ...
  const note=$("fundflowDetailNote");if(note)note.textContent=`${d?.methodology?.factor||"因子熱度以 50 為中性。"}｜${d?.methodology?.impact||"公司貢獻為移除該公司後重算的座標差。"}｜${d?.methodology?.projection||"虛線與扇形是模型傾向，不是保證。"}`;
  renderFundflowChart();
}
async function loadFundflowDetail(tagId,force=false){
  ...
  fundflowDetailLoading=true;const card=$("fundflowDetailCard"),loading=$("fundflowDetailLoading");if(card)card.classList.remove("hidden");if(loading){loading.classList.remove("hidden");loading.textContent="拆解 X/Y、Phase、轉態扇形與公司貢獻…"}
  try{const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),22000);try{const res=await fetch(`/api/sync-status?view=fundflow-detail&tag=${encodeURIComponent(id)}&days=${fundflowTrajectoryDays}`,{cache:"no-store",signal:controller.signal});fundflowDetailData=await readJson(res,"題材拆解");renderFundflowDetail()}finally{clearTimeout(timer)}}catch(e){console.warn("XY/Phase 題材明細讀取失敗",e);if(loading){loading.classList.remove("hidden");loading.textContent=`明細讀取失敗：${e?.message||e}`}}finally{fundflowDetailLoading=false;if(loading&&fundflowDetailData?.tagId===id)loading.classList.add("hidden")}
}
