// StockZone v2.6.5.17 — official credit-trading history layer with adaptive accelerated 250/500D research backfill.
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      const r = await fetch(url, {
        ...
        redirect: 'follow',
        headers: {
          'User-Agent': 'Mozilla/5.0 (StockZone/2.6.5.17; Taiwan credit trading)',
...
async function runCreditTradingBackfill({ targetTradingDays = 500, maxNewDays = 24, maxRunMs = 45000, concurrency = 2 } = {}) {
  await ensureCreditTradingSchema();
  ...
  const workerCount = Math.max(1, Math.min(3, Number(concurrency) || 2));
    ...
    const worker = async () => {
      while (cursor < missing.length && attempts < maxAttempts && addedDates[market].length < perMarketBudget && Date.now() - started < maxRunMs) {
        ...
        try {
          ...
          // A date is complete only when both margin/short and SBL families exist.
          ...
          } else errors.push({ market, tradeDate: iso, error: `partial credit day: margin=${hasMargin}, sbl=${hasSbl}` });
