...
function listStocksByCode(){
 const byCode=new Map();
 for(const type of ["holdings","watchlist"])for(const x of readList(type))if(x.code){
  const code=String(x.code),prev=byCode.get(code);
  if(!prev){byCode.set(code,x);continue}
  const prevQuote=new Date(prev.quoteUpdatedAt||0).getTime()||0,nextQuote=new Date(x.quoteUpdatedAt||0).getTime()||0;
  const prevTarget=new Date(prev.targetUpdatedAt||0).getTime()||0,nextTarget=new Date(x.targetUpdatedAt||0).getTime()||0;
  byCode.set(code,{...prev,...x,quoteUpdatedAt:prevQuote&&nextQuote?new Date(Math.min(prevQuote,nextQuote)).toISOString():(prev.quoteUpdatedAt||x.quoteUpdatedAt),targetUpdatedAt:prevTarget&&nextTarget?new Date(Math.min(prevTarget,nextTarget)).toISOString():(prev.targetUpdatedAt||x.targetUpdatedAt)});
 }
 return byCode;
}
function patchListStock(code,patch){
...
async function autoRefreshQuotes(force=false){
 ...
 try{
  const due=[...listStocksByCode()].filter(([,base])=>force||!base.quoteUpdatedAt||Date.now()-new Date(base.quoteUpdatedAt).getTime()>=AUTO_QUOTE_MS);
  let next=0;
  async function worker(){
   while(next<due.length){
    const [code,base]=due[next++];
    try{
     const d=await quote(code);
     patchListStock(code,{last:Number(d.last??d.price??d.regularMarketPrice),name:shortStockName(d.name||d.shortName||base.name),quoteUpdatedAt:new Date().toISOString()});
     renderLists();
    }catch(e){console.warn("清單股價更新失敗",code,e)}
   }
  }
  await Promise.all(Array.from({length:Math.min(3,due.length)},()=>worker()));
...
async function autoRefreshTargets(force=false){
 ...
 try{
  for(const [code,base] of listStocksByCode()){
   ...
   try{
    const payload=await fetchTargetPayload(code,base.name||""),fresh=normalizeTargetRows(payload),cache=readTargetCache(),old=cache[code]?.rows||[],merged=mergeTargetRows(old,fresh),main=pickMainTarget(merged),now=new Date().toISOString();
    cache[code]={rows:merged,updatedAt:now};writeTargetCache(cache);
    patchListStock(code,{target:main?targetPriceValue(main.latest):base.target,targetBroker:main?targetBrokerName(main.row):base.targetBroker,targetUpdatedAt:now});
    renderLists();
   }catch(e){console.warn("清單目標價更新失敗",code,e)}
  }
 }finally{autoTargetRefreshing=false}
}
setTimeout(()=>autoRefreshQuotes(true),800);
setTimeout(()=>autoRefreshTargets(false),5000);
