...
function parseDispositionPeriod(value='',text=''){const v=String(value||''),all=`${v} ${text||''}`;let dates=dispositionDates(v);if(dates.length>=2)return {start:dates[0],end:dates.at(-1)};dates=dispositionDates(all);if(dates.length>=2)return {start:dates[0],end:dates.at(-1)};return {start:'',end:''}}
function disposalPeriodOf(row){if(row?.periodStart&&row?.periodEnd)return {start:row.periodStart,end:row.periodEnd};const raw=row?.raw||{};return parseDispositionPeriod(raw.DispositionPeriod||raw.DisposalPeriod||raw['處置起訖時間']||raw['處置起迄時間']||'',row?.text||'')}
...
async function tpex(code){
  const safe=async(url,retries=0,timeoutMs=3200)=>{try{return {ok:true,data:await jfetch(url,retries,timeoutMs),source:url}}catch(e){return {ok:false,data:[],error:e?.message||'fetch failed',source:url}}};
  ...
  // 處置清單不可只信單一 endpoint。OpenAPI 與官網公告 JSON 每次都同時抓取後合併，避免其中一邊 HTTP 200 但資料暫時不完整而漏股。
  const [attR,openDisR,legacyR,noteR]=await Promise.all([safe(attUrl),safe(disUrl),safe(legacyUrl,0,4500),safe(noteUrl)]);
  ...
  const normalize=(list,source)=>{const out=[];for(const x of Array.isArray(list)?list:[]){const sc=stockCode(x);if(!sc)continue;const periodRaw=val(x,['DispositionPeriod','DisposalPeriod','DispositionTime','處置起訖時間','處置起迄時間','處置期間']);const text=clean(val(x,['DisposalCondition','DispositionReasons','DispositionReason','DisposalInformation','處置原因','處置條件','處置內容','處置措施']));const p=parseDispositionPeriod(periodRaw,text);out.push({date:rocToIso(val(x,['Date','AnnouncementDate','公布日期','公告日期','處置日期'])),text,raw:{...x,DispositionPeriod:periodRaw||x?.DispositionPeriod||'',_source:source},periodStart:p.start,periodEnd:p.end,stockCode:sc})}return out};
  const openUsable=openDisR.ok&&Array.isArray(openDisR.data),legacyNormalized=legacyR.ok?legacyTpexRows(legacyR.data):[],legacyUsable=legacyR.ok&&Array.isArray(legacyNormalized);
  const merged=[...normalize(openDisR.data,disUrl),...normalize(legacyNormalized,legacyUrl)];
  const seen=new Set(),allPunish=merged.filter(x=>{const k=`${x.stockCode}|${x.periodStart}|${x.periodEnd}|${x.text}`;if(seen.has(k))return false;seen.add(k);return true});
  let punish=allPunish.filter(x=>x.stockCode===String(code));
  let htmlSource='',htmlError='';
  // 只有其中一條主要處置資料源失敗、且合併後沒有這檔時，再用 TPEx 官方公告 HTML 做 code-specific 核對；避免把單一路徑漏資料誤判成未處置。
  if(!punish.length&&(!openUsable||!legacyUsable)){
    try{const html=await tfetch('https://app.tpex.org.tw/HTML/Announce/MarketNews/disposal_info.html',0,2800),parsed=parseTpexDisposalHtml(html,code);if(parsed.length){punish=parsed.map(x=>{const p=disposalPeriodOf(x);return {...x,periodStart:p.start,periodEnd:p.end,stockCode:String(code)}});htmlSource='https://app.tpex.org.tw/HTML/Announce/MarketNews/disposal_info.html'}else htmlError='官方 HTML 備援沒有解析到該股票處置資料'}catch(e){htmlError=e?.message||'HTML backup failed'}
  }
  punish.sort((a,b)=>String(b.periodStart||'').localeCompare(String(a.periodStart||''))||String(b.date||'').localeCompare(String(a.date||'')));
  ...
  const disposalAvailable=openUsable||legacyUsable||Boolean(htmlSource);
  const disposalSources=[openUsable?disUrl:'',legacyUsable?legacyUrl:'',htmlSource].filter(Boolean).join(' + ');
  return {rows,punish,activePunish,warning,historyComplete:false,availability:{attention:attR.ok,disposal:disposalAvailable,warning:noteR.ok},sources:{attention:attR.source,disposal:disposalSources||disUrl,warning:noteR.source},errors:{attention:attR.error||'',disposal:openDisR.error||'',disposalBackup:legacyR.error||'',disposalHtmlBackup:htmlError,warning:noteR.error||''}};
}
...
function disposalPeriodLabel(punish=[]){
 for(const row of punish||[]){const p=disposalPeriodOf(row);if(p.start&&p.end)return `${p.start.replace(/-/g,'/')}～${p.end.replace(/-/g,'/')}`}
 return '處置期間依官方公告';
}
function activeDisposalPeriod(punish=[]){for(const row of punish||[]){const p=disposalPeriodOf(row);if(p.start&&p.end)return {start:p.start,end:p.end,label:`${p.start.replace(/-/g,'/')}～${p.end.replace(/-/g,'/')}`}}return null}
...
export default async function handler(req,res){
 try{
  ...
  const currentDisposalPeriod=activeDisposalPeriod(activePunish);
