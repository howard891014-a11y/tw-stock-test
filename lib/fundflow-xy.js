// StockZone v2.6.4.2 — Market-Topic Fund-Flow XY Engine v2.1.2 + Phase Transition Engine v1
...
const ENGINE_VERSION = 'xy-2.1.2-market-topic-phase-1.0';
...
function buildBusinessBrowserCatalog(profiles=[],snapshot={}){
  ...
  for(const p of profiles||[]){
    ...
    const marketLinks=marketTopicLinks(resolved.tags||[],companyName,companyCode);
...
function buildProfileTagMap(profiles){
  ...
  for(const p of profiles||[]){
    ...
    const links=marketTopicLinks(resolved.tags||[],resolved.name||p.stock_name||p.name||'',resolved.symbol||code).map(link=>{
...
async function loadEngineInputs(sql,trajectoryDays){
  const profiles=await sql.query(`SELECT stock_code,stock_name,market,industry_code,industry,auto_business_tags,main_business,business_enrich_status,business_enrich_checked_at FROM market_company_profile ORDER BY stock_code`);
