...
const NEWS_CACHE_KEY="stockzone_news_cache_v2505";
const NEWS_MATCH_CACHE_KEY="stockzone_news_match_cache_v2505";
...
function newsMergeKey(x){return `${String(x?.title||"").replace(/\s+/g," ").trim()}|${String(x?.publishedAt||x?.date||"").slice(0,16)}|${String(x?.source||"").trim()}`}
function blockedNewsItem(x){return /股市爆料同學會|同學風向與貼文摘要/.test(`${x?.title||""} ${x?.source||""}`)}
function mergeNews(oldRows,newRows){const m=new Map();[...(oldRows||[]),...(newRows||[])].forEach(x=>{if(x?.title&&!blockedNewsItem(x))m.set(newsMergeKey(x),x)});const stars=newsSet(NEWS_STAR_KEY);const rows=[...m.values()].sort((a,b)=>(stars.has(newsKey(b))-stars.has(newsKey(a)))||(newsTime(b)-newsTime(a)));const pinned=rows.filter(x=>stars.has(newsKey(x))),normal=rows.filter(x=>!stars.has(newsKey(x))).slice(0,80);return [...pinned,...normal]}
...
async function fetchNewsMode(code,name,mode,forceReset=false){
 ...
 try{const r=await fetch(`/api/news?code=${encodeURIComponent(code||"")}&name=${encodeURIComponent(name||"")}&mode=${mode}${terms?`&terms=${encodeURIComponent(terms)}`:""}&v=2505`,{cache:"no-store",signal:controller.signal});const data=await readJson(r,"新聞");const incoming=(data.items||[]).filter(x=>!blockedNewsItem(x)),oldKeys=new Set(cached.map(newsMergeKey)),unread=newsSet(NEWS_UNREAD_KEY,code);if(cached.length)for(const x of incoming)if(x?.title&&!oldKeys.has(newsMergeKey(x)))unread.add(newsKey(x));saveNewsSet(NEWS_UNREAD_KEY,unread,code);const merged=mergeNews(cached,incoming);all[String(code)]={rows:merged,query:terms,updatedAt:new Date().toISOString()};writeNewsStore(key,all);if(mode==="match")newsMatchRowsCache=merged;else newsRowsCache=merged;renderNews();return true}finally{clearTimeout(timer)}
}
