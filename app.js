...
function correctedHistoryItem(code,parent,h){
  ...
  const key=base._correctionKey||correctionKey(code,base),c=corrections[key];
...
function correctedRow(code,row){
  if(!row)return row;
  const rawHistory=(row.targetHistory?.length?row.targetHistory:[normalizeHistoryItem(row)]).slice(0,5);
...
function historyRows(h,start=0,stockCode="",type="",index=-1,parentBroker=""){
  return h.map((y,i)=>{
    ...
    const brokerNote=(y.manualBroker||((y.broker||"")&&y.broker!==parentBroker))?`<span class="history-broker">${esc(y.broker||parentBroker)}${y.manualBroker?` <button class="manual-edit compact" data-manual-field="broker" data-row="${payload}" data-type="${type}" data-i="${index}" title="修改或刪除手動修正">✎</button>`:""}</span>`:"";
    ...
    return`<div class="history-item"><span>${start+i===0?"最新":start+i===1?"前次":`第${start+i+1}筆`}</span><strong>${fmt(y.target)}${targetEdit}</strong><span>${date(y.date)}</span><span class="history-actions">${y.sourceUrl?`<a href="${esc(y.sourceUrl)}" target="_blank" rel="noopener">來源</a>`:""}<button class="repair-button compact" data-repair data-row="${payload}" data-type="${type}" data-i="${index}">修正</button></span>${brokerNote}</div>`;
  }).join("");
}
function historyHtml(x,stockCode="",type="",index=-1){const h=(x.targetHistory?.length?x.targetHistory:[normalizeHistoryItem(x)]).slice(0,5),shown=h.slice(0,2),older=h.slice(2);return`<div class="history-list">${historyRows(shown,0,stockCode,type,index,x.broker||"")}</div>${older.length?`<details class="target-history"><summary>較早目標價（${older.length}）</summary>${historyRows(older,2,stockCode,type,index,x.broker||"")}</details>`:""}`}
