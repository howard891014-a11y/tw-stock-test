// StockZone v2.6.5.17 — A-plan Market-Topic Fund-Flow XY Engine v3.0.0 + Phase Transition Engine v1
...
const ENGINE_VERSION = 'xy-3.0.0-a-xv2-institutional-credit';
  ...
  for(const list of byCode.values()){
    ...
    for(let i=0;i<list.length;i++){
      ...
      const rolling=(days,key='inst_net_value')=>{
        ...
        return {ratio:value>0?flow/value*100:null,value:valid?flow:null,days:valid};
      };
      ...
      row.inst_flow_value_5=r5.value;row.inst_flow_days_20=r20.days;
...
function summarizeTagItems(items,denom){
  ...
  const flowItems=items.filter(x=>num(x.inst_flow_ratio_5)!==null||num(x.inst_flow_ratio_1)!==null),allWeight=items.reduce((sum,x)=>sum+Math.max(0,num(x.weight)??0),0),flowWeight=flowItems.reduce((sum,x)=>sum+Math.max(0,num(x.weight)??0),0),flowCoveragePct=allWeight>0?flowWeight/allWeight*100:0;
  // Missing institutional rows are not bearish.  Treat no evidence as neutral and
  // shrink the formal X toward 50 according to institutional coverage.
  ...
  const positiveFlows=flowItems.map(x=>Math.max(0,(num(x.inst_flow_value_5)??0)*(num(x.weight)??1))).filter(x=>x>0),positiveTotal=positiveFlows.reduce((s,x)=>s+x,0),topPositive=positiveFlows.length?Math.max(...positiveFlows):0;
  ...
  const baseX=.40*xMedian+.25*xMean+.25*flowBreadth+.10*concentrationQuality;
  const baseY=.45*yMedian+.25*yMean+.20*priceBreadth+.10*upBreadth;
  const sampleReliability=Math.min(1,Math.sqrt(validCount/5)),coverageReliability=Math.min(1,coveragePct/80),reliability=sampleReliability*coverageReliability,flowCoverageReliability=Math.min(1,flowCoveragePct/80),xReliability=reliability*flowCoverageReliability;
  const institutionalX=clamp(50+(baseX-50)*xReliability),groupCreditCorrection=clamp((weightedMean(items,'creditCorrection')??0)*xReliability,-15,15),x=clamp(institutionalX+groupCreditCorrection),y=clamp(50+(baseY-50)*reliability);
  ...
  const factorSignals={
    x:{inst1:round(weightedMean(items,'rankInst1')??50),inst5:round(weightedMean(items,'rankInst5')??50),inst10:round(weightedMean(items,'rankInst10')??50),inst20:round(weightedMean(items,'rankInst20')??50),streak:round(weightedMean(items,'rankInstStreak')??50),agreement:round(weightedMean(items,'rankInstAgreement')??50),flowBreadth:round(flowBreadth),concentrationQuality:round(concentrationQuality),creditQuality:round(clamp(50+groupCreditCorrection/15*50)),institutionalBase:round(institutionalX),flowCoverage:round(flowCoveragePct)},
  ...
  return {memberCount,validCount,coveragePct:round(coveragePct),reliability:round(reliability*100),x:round(x,2),y:round(y,2),rawX:round(x,2),rawY:round(y,2),
    institutionalX:round(institutionalX,2),creditCorrection:round(groupCreditCorrection,2),legacyX:round(legacyX,2),flowCoveragePct:round(flowCoveragePct),flowBreadth:round(flowBreadth),concentrationQuality:round(concentrationQuality),topFlowSharePct:round(topFlowSharePct),
...
function aggregateTagDate(scoredRows,profileMaps,date){
  ...
  const out=[];for(const [tagId,items] of groups){const meta=profileMaps.tagMeta.get(tagId);if(!meta)continue;const denom=profileMaps.denominator.get(tagId)||{memberCount:items.length,totalWeight:items.reduce((s,x)=>s+x.weight,0)},summary=summarizeTagItems(items,denom);if(!summary)continue;
    const leaders=items.slice().sort((a,b)=>(b.stockX+b.stockY)-(a.stockX+a.stockY)).slice(0,5).map(a=>({code:String(a.stock_code),name:a.companyName||a.stock_name||'',market:a.market,x:round(a.stockX),y:round(a.stockY),changePct:round(num(a.change_pct)||0,2),return5Pct:round(num(a.return_5_pct)||0,2),instFlow5Pct:round(num(a.inst_flow_ratio_5)||0,3),creditCorrection:round(num(a.creditCorrection)||0,1),importance:a.importance}));
...
function computeTagDetail(profiles,activityRows,tagId,{maxDates=10}={}){
  ...
  return {ok:true,engineVersion:ENGINE_VERSION,tagId:cleanTag,name:meta.name,parentName:meta.parentName||'',scope:meta.scope,asOf:latestPoint.date,trajectoryDays:enrichedTrajectory.length,trajectory:enrichedTrajectory,
    latest:{x:g.x,y:g.y,rawX:g.rawX,rawY:g.rawY,institutionalX:g.institutionalX,creditCorrection:g.creditCorrection,legacyX:g.legacyX,quadrant:g.quadrant,status:g.status,statusLabel:g.statusLabel,phaseState:g.phaseState,phaseLabel:g.phaseLabel,phaseConfidence:g.phaseConfidence,confirmation:g.confirmation,overheating:g.overheating,
      ...
      groupBuild:{xMedian:latestRawSummary.xMedian,xMean:latestRawSummary.xMean,flowBreadth:latestRawSummary.flowBreadth,concentrationQuality:latestRawSummary.concentrationQuality,institutionalX:latestRawSummary.institutionalX,creditCorrection:latestRawSummary.creditCorrection,legacyX:latestRawSummary.legacyX,yMedian:latestRawSummary.yMedian,yMean:latestRawSummary.yMean,priceBreadth:latestRawSummary.priceBreadth,upBreadth:latestRawSummary.upBreadth,reliability:latestRawSummary.reliability}},
    ...
    methodology:{impact:'公司貢獻＝移除該公司後重新計算當日原始業務座標，與完整原始座標的差。',factor:'X v2 以法人淨資金流為主體，信用交易最多只做 ±15 分籌碼品質修正；因子熱度 50 約為全市場中性。',projection:'扇形是狀態＋速度＋歷史同狀態轉態分布的不確定範圍，不是未來保證。'}};
}
...
async function getFundflowSnapshot({days=10,force=false}={}){
  ...
  const value={ok:true,engineVersion:ENGINE_VERSION,asOf:allDates.at(-1),trajectoryDates:allDates.slice(-bounded),trajectoryDays:Math.min(bounded,allDates.length),
    ...
    methodology:{stockX:'X v2 主體＝法人淨買賣超金額／成交值的 1/5/10/20 日全市場百分位（15/30/25/20%）＋連買賣持續性 5%＋外資/投信/自營方向一致度 5%；信用交易只允許 ±15 分修正。',stockY:'10% 當日漲跌＋25% 3日報酬＋30% 5日報酬＋20% 20日報酬＋15% 近5日上漲持續性；價格因子採全市場百分位。',group:'業務 X 先聚合法人分數的中位數／平均、法人淨流入廣度與流入集中度，再套用信用籌碼修正；小樣本向 50 收縮，座標用 3 交易日 EMA 平滑。Legacy 成交活動 X 同步保存但不再主導正式圖。',phase:'C＝族群同步確認度；E＝高檔過熱／衰竭度；Phase 另結合 ΔX/ΔY 與加速度並使用兩日狀態黏性。',projection:'Phase、ΔXY 與歷史同狀態轉態分布形成模型傾向；點開題材顯示不確定扇形。',caution:'X v2 是官方法人淨流向的相對強弱模型，信用交易是籌碼品質修正；不是逐筆成交資金流，也不是未來保證。'},
