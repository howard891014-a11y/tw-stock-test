// StockZone v2.6.5.24 — Self-Relative Market-State XY Engine v5.0
// X = sign(raw 5D institutional net-flow / turnover) × that magnitude's rolling percentile versus the SAME topic's own recent history.
//     Right/left therefore still means literal institutional net-buy/net-sell; distance from 0 means how unusual that pressure is for this topic itself.
// Y = literal business-weighted 5-session constituent price return. Price remains the primary profit axis.
// rawX keeps the original 5D institutional net-flow/turnover percentage for audit/tooltips. No future dates are used in the rolling X baseline.
// Breadth, streak, agreement, credit trading and reliability do not alter XY coordinates; they remain C/E/Phase/Future-Path diagnostics.
...
const ENGINE_VERSION = 'xy-5.0.0-self-relative-flow-price';
const DEFAULT_TRAJECTORY_DAYS = 15;
const ENGINE_HISTORY_DAYS = 60;
const SELF_RELATIVE_LOOKBACK_DAYS = 60;
const SELF_RELATIVE_MIN_HISTORY_DAYS = 20;
const PREPARED_SNAPSHOT_DAYS = Object.freeze([5,10,15]);
const IMPORTANCE_WEIGHT = Object.freeze({ core:1, important:0.5, related:0.2 });
const NON_CORE_EXPOSURE_BUDGET = null; // compatibility export only; v5 XY has no cross-topic cap
...
function empiricalMagnitudePercentile(current,history){
  const c=Math.abs(num(current)??0),values=(history||[]).map(v=>Math.abs(num(v)??0)).filter(Number.isFinite);
  if(!values.length)return 0;
  let less=0,equal=0;for(const v of values){if(v<c)less++;else if(Math.abs(v-c)<1e-12)equal++;}
  // A non-zero raw flow must stay on its literal buy/sell side of the axis.
  // The half-observation plotting position prevents the smallest magnitude from
  // becoming exactly 0 while still allowing a new historical extreme to reach 100.
  return clamp((less+0.5*equal+0.5)/(values.length+0.5)*100);
}
function applySelfRelativeX(rowsByDate,dates,{lookback=SELF_RELATIVE_LOOKBACK_DAYS,minHistory=SELF_RELATIVE_MIN_HISTORY_DAYS}={}){
  // Historical point t may only compare with observations available BEFORE t.
  // This keeps walk-forward/backtests causal and prevents the current observation
  // from helping set its own percentile.
  const historyByTag=new Map(),maxLookback=Math.max(5,Number(lookback)||SELF_RELATIVE_LOOKBACK_DAYS),warmup=Math.max(5,Number(minHistory)||SELF_RELATIVE_MIN_HISTORY_DAYS);
  for(const date of dates||[]){
    for(const row of rowsByDate.get(date)||[]){
      const raw=num(row.rawX)??num(row.x)??num(row.themeFlow5Pct)??0,tagId=String(row.tagId||''),prior=historyByTag.get(tagId)||[],baseline=prior.slice(-maxLookback);
      const pct=raw===0||!baseline.length?0:empiricalMagnitudePercentile(raw,baseline),maturity=Math.min(1,baseline.length/warmup),relative=raw===0?0:sign(raw)*pct*maturity;
      row.rawX=round(raw,4);row.institutionalX=round(raw,4);row.x=round(relative,2);row.baseX=row.x;row.selfRelativeXPercentile=round(pct,1);row.xHistoryDays=baseline.length;row.xMaturityPct=round(maturity*100,1);
      if(row.factorSignals?.x){row.factorSignals.x.selfRelative=round(relative,2);row.factorSignals.x.selfPercentile=round(pct,1);row.factorSignals.x.historyDays=baseline.length;row.factorSignals.x.maturityPct=round(maturity*100,1);row.factorSignals.x.rawInst5=round(raw,4);}
      prior.push(raw);if(prior.length>maxLookback)prior.splice(0,prior.length-maxLookback);historyByTag.set(tagId,prior);
    }
  }
  return rowsByDate;
}
...
// Stock-level raw institutional flow remains independent of price and credit.
// Credit/market breadth stay diagnostic; only the topic-level rolling transform
// converts raw institutional pressure into the self-relative X coordinate.
...
function rawPhaseForPoint(p){
  ...
  if(y>=5&&e>=58)key='overheating';else if(y>0&&((dx3<-12&&dy3<-.8)||(e>=70&&dx1<0&&dy1<=0)))key='cooling';else if(x>=45&&y>0&&c>=35)key='mainline';else if(x>=45&&y<=0&&dx3>=-12&&dy3>=-2.5&&c>=28)key='potential';else if(y<=1.5&&((x>0&&dx3>10)||(x>65&&dx1>=0)))key='germination';else if(x<0&&y<0)key='cold';
  const m=phaseMeta(key),xs=flowPhaseScore(x),ys=pricePhaseScore(y),mx=clamp(50+dx3*1.25),my=clamp(50+dy3*12);let confidence=45;
  if(key==='cold')confidence=clamp(35+(50-xs)*.55+(50-ys)*.45+c*.12);else if(key==='germination')confidence=clamp(38+Math.max(0,mx-50)*.65+Math.max(0,x)*.15+c*.18);else if(key==='potential')confidence=clamp(40+Math.max(0,x)*.16+Math.max(0,50-ys)*.25+Math.max(0,my-40)*.2+c*.20);else if(key==='mainline')confidence=clamp(42+Math.max(0,x)*.14+Math.max(0,y)*3+c*.22-Math.max(0,e-70)*.25);else if(key==='overheating')confidence=clamp(42+Math.max(0,y-5)*5+e*.38);else if(key==='cooling')confidence=clamp(42+Math.max(0,-dx3)*1.3+Math.max(0,-dy3)*7+e*.30);else confidence=clamp(32+c*.25-Math.abs(mx-my)*.08);
...
function computeBusinessFlow(profiles,activityRows,{maxDates=DEFAULT_TRAJECTORY_DAYS}={}){
  ...
  applySelfRelativeX(aggregated,dates);
...
function computeTagDetail(profiles,activityRows,tagId,{maxDates=10}={}){
  ...
  applySelfRelativeX(rowsByDate,dates);
  ...
  const companies=latestItems.map((item)=>{
    const code=String(item.stock_code||''),reducedScored=latestScored.filter(x=>String(x.stock_code||'')!==code),reducedGroup=aggregateTagDate(reducedScored,profileMaps,latestDate).find(x=>x.tagId===cleanTag),withoutX=reducedGroup?.rawX??reducedGroup?.x??0,withoutY=reducedGroup?.rawY??reducedGroup?.y??0,impactX=(num(latestRawSummary.rawX)??0)-withoutX,impactY=(num(latestRawSummary.rawY)??0)-withoutY;return {
  ...
  return {ok:true,engineVersion:ENGINE_VERSION,tagId:cleanTag,name:meta.name,parentName:meta.parentName||'',scope:meta.scope,asOf:latestPoint.date,trajectoryDays:enrichedTrajectory.length,trajectory:enrichedTrajectory,
    ...
    methodology:{impact:'公司 X 貢獻以移除該公司後的 raw 5 日法人流入比例差表示；因 self-relative percentile 是非線性座標，不把單家公司硬拆成 percentile 點數。Y 貢獻仍是移除後的 5 日價格報酬差。',factor:'X＝sign(raw 5日法人淨買賣超／成交值) × 該強度相對題材自身最多近60個先前交易日的百分位；右＝買、左＝賣，離中心越遠代表對該題材自己越異常。Y＝題材近5日實際價格漲跌幅。',projection:'Future Path 採 Top-2 劇本：只保留信心最高的 A/B 兩條 3/5/10 日路徑，各自畫獨立颱風走廊；A-B 信心差作為混沌度的重要訊號。'}};
}
...
function buildPreparedOverview(fullGroups,allDates,calibration,days){
  ...
  return {ok:true,engineVersion:ENGINE_VERSION,asOf:allDates.at(-1),trajectoryDates:allDates.slice(-bounded),trajectoryDays:Math.min(bounded,allDates.length),preparedSnapshot:true,
    axes:{x:'法人相對自身歷史強度',y:'近5日題材價格漲跌幅',center:0,xUnit:'relative-percentile',yUnit:'%',window:5,xLookback:SELF_RELATIVE_LOOKBACK_DAYS},
    methodology:{stockX:'raw X 仍是近5日三大法人淨買賣超金額占成交值比例；畫面 X 再保留 raw 正負號，並以該題材自己最多近60個『先前交易日』的 raw X 絕對強度百分位決定離中心距離。',stockY:'Y 只描述題材成分公司近5日實際報酬的業務關聯加權平均；價格是主要獲利方向，不加入廣度、持續性或可靠度修正。',group:'核心／重要／相關只作題材內業務關聯權重 1.0／0.5／0.2。先算 raw X＝Σ(業務權重×近5日法人淨買賣超金額) ÷ Σ(業務權重×近5日成交值)，再算 X＝sign(raw X)×percentile(|raw X|，題材自身最多近60個先前交易日)。Y＝Σ(業務權重×個股近5日報酬) ÷ Σ業務權重。',phase:'Y 決定價格是否真的轉強／轉弱；X 用來判斷這段價格走勢是否有相對自身歷史異常的法人資金支持。廣度、三法人一致度、streak、資料可靠度與信用交易只留在 C/E、籌碼品質、Phase 與 Future Path。',projection:'Phase、ΔXY 與歷史同狀態轉態分布產生 Top-2 未來劇本；A/B 各自保留 3/5/10 日路徑與不確定走廊。',caution:'X 的正負仍是實際法人買／賣，數值大小是相對自身歷史強度，不是百分比。raw X 仍保留供查核；Y 是題材成分股業務權重加權的近5日價格報酬。'},
...
async function loadTagEngineInputs(sql,tagId,trajectoryDays){
  ...
  `,[Math.max(SELF_RELATIVE_LOOKBACK_DAYS+1,Math.max(Number(trajectoryDays)||10,5)+20)]);
  const dates=dateRows.map(x=>isoDate(x.trade_date)).filter(Boolean).sort();if(!dates.length)return {profiles,dates,activityRows:[]};
  const activityRows=await sql.query(`
    SELECT a.trade_date,a.stock_code,a.stock_name,a.market,a.trade_value,a.change_pct,a.value_ratio_20,a.value_trend_5_15,a.up_value_share_5,a.positive_days_5,a.return_3_pct,a.return_5_pct,a.return_20_pct,
      h.close_price,h.trade_volume,
...
module.exports={ENGINE_VERSION,DEFAULT_TRAJECTORY_DAYS,ENGINE_HISTORY_DAYS,SELF_RELATIVE_LOOKBACK_DAYS,SELF_RELATIVE_MIN_HISTORY_DAYS,PREPARED_SNAPSHOT_DAYS,PHASES,NON_CORE_EXPOSURE_BUDGET,applyCompanyExposureBudget,percentileRanks,percentileRanksNullable,empiricalMagnitudePercentile,applySelfRelativeX,enrichFlowFeatures,scoreStocksForDate,summarizeTagItemsRaw,normalizeTagSummaries,aggregateTagDate,computeBusinessFlow,computeTagDetail,quadrant,buildTransitionCalibration,projectGroup,ensureBusinessFlowSchema,refreshBusinessFlowDaily,getFundflowSnapshot,getFundflowDetail,getFundflowBusinessBrowser,buildBusinessBrowserCatalog,buildPreparedOverview,compactOverviewGroup,loadTagEngineInputs,rebuildPreparedFromStored};
