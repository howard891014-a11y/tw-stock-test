...
const COMPANIES = Object.freeze([
  ...
  company("6207", "雷科", "electronics_services", [biz("glass_substrate", "important")], { note:"TGV／玻璃基板雷射鑽孔設備題材映射" }),
  company("8027", "鈦昇", "semiconductor", [biz("glass_substrate", "important")], { note:"TGV／玻璃基板雷射與製程設備題材映射" }),
  company("7828", "創新服務", "semiconductor", [biz("glass_substrate", "important")], { note:"TGV／玻璃核心基板製程題材映射" }),
  ...
  // v2.6.2.33-C6 — final four TDR profiles that use special industry code 91.
  // These are curated from the issuers' disclosed main-business descriptions, so they
  // resolve directly without falling into the generic "unclassified" bucket.
  company("910861", "神州-DR", "software", [
    biz("ai_solution", "core"),
    biz("digital_platform", "important"),
    biz("system_integration", "important"),
    biz("software_development", "important")
  ], { note:"數據智能決策/AI 全棧技術、數位供應鏈與金融科技服務" }),
  company("9110", "越南控-DR", "motorcycle", [
    biz("motorcycle", "core")
  ], { note:"機車、機車零組件與相關金屬零件製造" }),
  company("911608", "明輝-DR", "electronics_services", [
    biz("cybersecurity", "core"),
    biz("system_integration", "important"),
    biz("led_lighting", "important"),
    biz("infrared_thermal_sensor", "important")
  ], { note:"電氣/技術供應、整合工程、資安、LED 照明與紅外線熱感測" }),
  company("9136", "巨騰-DR", "computing", [
    biz("computer_chassis", "core"),
    biz("consumer_electronics", "important")
  ], { note:"筆記型電腦外殼與手持設備外殼" }),
...
function parseAutoBusinessTags(value){
  if(Array.isArray(value))return value.map(String).map(x=>x.trim()).filter(Boolean);
  if(value&&typeof value==='object'){
    if(Array.isArray(value.tags))return value.tags.map(String).map(x=>x.trim()).filter(Boolean);
    return [];
  }
  const raw=String(value||'').trim();if(!raw)return [];
  try{const j=JSON.parse(raw);return Array.isArray(j)?j.map(String).map(x=>x.trim()).filter(Boolean):[];}catch{return []}
}
...
function resolveCompanyBusinessTags(profile={}, options={}){
  ...
  const autoIds=parseAutoBusinessTags(profile.autoBusinessTags??profile.auto_business_tags);
  for(const id of autoIds)businessLinks.push({id,importance:"related",origin:"mops-business"});
  ...
  let resolution=exact?"curated":seedIds.length?"official-chain":autoIds.length?"mops-business":"none";
...
function summarizeProfiles(profiles=[]){
  ...
  let crossIndustryTechCompanies=0,curated=0,officialChain=0,mopsBusiness=0,industryFallback=0,coarseIndustry=0,unmappedAll=0;
  ...
  for(const p of profiles){
    ...
    else if(resolved.resolution==="mops-business")mopsBusiness++;
  ...
  return Object.freeze({
    ...
    fineTechCompanies,
    curated,officialChain,mopsBusiness,industryFallback,coarseIndustry,unmappedAll,
...
module.exports=Object.freeze({
  ...
  getCompany,getCompanyByName,isTechnologyIndustry,fallbackTagForIndustry,broadTagForIndustry,parseAutoBusinessTags,
